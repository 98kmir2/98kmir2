...
resourcestring
  sDBHeaderDesc = '98K帐号数据库文件 20191007';
  sDBIdxHeaderDesc = '98K帐号数据库索引文件 20191007';
type
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        g_n472A74 := DBHeader.nIDCount;
        for nIndex := 0 to DBHeader.nIDCount - 1 do
        begin
          Inc(g_n472A6C);
          if FileRead(m_nFileHandle, DBRecord, SizeOf(TAccountDBRecord)) <> SizeOf(TAccountDBRecord) then
          begin
            Break;
          end;
          if not DBRecord.Header.boDeleted then
          begin
            if DBRecord.UserEntry.sAccount <> '' then
            begin
...
begin
  for I := 0 to m_QuickList.Count - 1 do
  begin
    if CompareLStr(m_QuickList.Strings[I], sName, Length(sName)) then
    begin
...
function TFileIDDB.GetRecord(nIndex: Integer;
  var DBRecord: TAccountDBRecord): Boolean;
begin
  if FileSeek(m_nFileHandle, SizeOf(TAccountDBRecord) * nIndex + SizeOf(TDBHeader), 0) <> -1 then
  begin
...
begin
  ...
  if GetRecord(nRecordIndex, DBRecord) then
    Result := nRecordIndex
  else
    Result := -1;
end;
...
begin
  ...
  begin
    ...
    if boNew then
    begin
      if FileRead(m_nFileHandle, DeletedHeader, SizeOf(TRecordDeletedHeader)) = SizeOf(TRecordDeletedHeader) then
      begin
        if DeletedHeader.boDeleted then
        begin
...
begin
  ...
  if m_QuickList.Count <= nIndex then
    Exit;
  if UpdateRecord(Integer(m_QuickList.Objects[nIndex]), DBRecord, False) then
    Result := true;
end;
...
begin
  ...
  if m_QuickList.GetIndex(sAccountName) >= 0 then
  begin
    Result := False;
  end
  else
  begin
    ...
    if UpdateRecord(nC, DBRecord, true) then
    begin
