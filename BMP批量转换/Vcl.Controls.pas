...
var
  _NewLeft, _NewTop, _NewWidth, _NewHeight: Integer;
begin
  with Rect do
  begin
    ...
    begin
      with AControl do
        ...
        begin
          _NewLeft := Margins.ControlLeft;
          _NewTop := Margins.ControlTop;
          _NewWidth := Margins.ControlWidth;
          _NewHeight := Margins.ControlHeight;
          if akRight in Anchors then                          // MulDiv(482, FCurrentPPI, 96);
            if akLeft in Anchors then                         // MulDiv(560, FCurrentPPI, 96);
            begin
              _NewWidth := ParentSize.X - (AControl.FOriginalParentSize.X - AControl.FAnchorRules.X);
            end
            else
              // The AnchorRules.X is the original left
              _NewLeft := ParentSize.X - (FOriginalParentSize.X - FAnchorRules.X)
          else if not (akLeft in Anchors) then
            // The AnchorRules.X is the original middle of the AControl
            _NewLeft := MulDiv(FAnchorRules.X, ParentSize.X, FOriginalParentSize.X) -
              _NewWidth div 2;
          if akBottom in Anchors then
            if akTop in Anchors then
              // The AnchorRules.Y is the original height
              _NewHeight := ParentSize.Y - (FOriginalParentSize.Y - FAnchorRules.Y)
            else
              // The AnchorRules.Y is the original top
              _NewTop := ParentSize.Y - (FOriginalParentSize.Y - FAnchorRules.Y)
          else if not (akTop in Anchors) then
            // The AnchorRules.Y is the original middle of the AControl
            _NewTop := MulDiv(FAnchorRules.Y, ParentSize.Y, FOriginalParentSize.Y) -
              _NewHeight div 2;
          if UpdateAnchorOrigin then
          begin
            if (Anchors * [akLeft, akRight]) <> [] then
              _NewLeft := MulDiv(FAnchorOrigin.X, ParentSize.X, FOriginalParentSize.X) - _NewWidth div 2;
            if (Anchors * [akTop, akBottom]) <> [] then
              _NewTop := MulDiv(FAnchorOrigin.Y, ParentSize.Y, FOriginalParentSize.Y) - _NewHeight div 2;
          end;
          Margins.SetControlBounds(_NewLeft, _NewTop, _NewWidth, _NewHeight, True);
        end;
      if AAlign = alNone then Exit;
    end;

    _NewWidth := Right - Left;
    if (_NewWidth < 0) or (AAlign in [alLeft, alRight, alCustom]) then
      _NewWidth := AControl.Margins.ControlWidth;
    _NewHeight := Bottom - Top;
    if (_NewHeight < 0) or (AAlign in [alTop, alBottom, alCustom]) then
      _NewHeight := AControl.Margins.ControlHeight;
    _NewLeft := Left;
    _NewTop := Top;
    case AAlign of
      alTop:
      ...
      alCustom:
        begin
          _NewLeft := AControl.Left;
          _NewTop := AControl.Top;
          CustomAlignPosition(AControl, _NewLeft, _NewTop, _NewWidth,
            _NewHeight, Rect, AAlignInfo);
        end;
    end;
  end;
  AControl.Margins.SetControlBounds(_NewLeft, _NewTop, _NewWidth, _NewHeight, True);
  { Adjust client rect if AControl didn't resize as we expected }
  if (AControl.Margins.ControlWidth <> _NewWidth) or (AControl.Margins.ControlHeight <> _NewHeight) then
    with Rect do
      case AAlign of
        alTop: Dec(Top, _NewHeight - AControl.Margins.ControlHeight);
        alBottom: Inc(Bottom, _NewHeight - AControl.Margins.ControlHeight);
        alLeft: Dec(Left, _NewWidth - AControl.Margins.ControlWidth);
        alRight: Inc(Right, _NewWidth - AControl.Margins.ControlWidth);
        alClient:
          begin
            Inc(Right, _NewWidth - AControl.Margins.ControlWidth);
            Inc(Bottom, _NewHeight - AControl.Margins.ControlHeight);
          end;
      end;
end;
...
begin
  ...
  begin
    ...
    begin
      LList := TList.Create;
      LList := TInterlocked.CompareExchange<TList>(FAsyncList, LList, nil);
      LList.Free;
    end;
    System.TMonitor.Enter(FAsyncList);
    try
