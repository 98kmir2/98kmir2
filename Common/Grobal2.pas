...
const
  MAX_SCREEN_PELS_WIDTH = 1600;                   //允许玩家选择最大的分辨率宽度  2019-12-25
  MAX_SCREEN_PELS_HEIGHT = 1200;                  //允许玩家选择最大的分辨率高度  2019-12-25
  CUSTOM_SAFEZONE_CONTROLSTART_INX = 10000;       //安全区控制代码起始索引  2019-12-16
  CUSTOM_SAFEZONE_CONTROLEND_INX = 19999;         //安全区控制代码结束索引  2019-12-16
  CUSTOM_SAFEZONE_EFFSTART_INX = 100;             //安全区光环特效类型起始索引   2019-12-5
  CUSTOM_SAFEZONE_EFFEND_INX = 199;               //安全区光环特效类型结束索引   2019-12-5
  CUSTOM_NPC_START_INX = 500;                     //自定义NPC起始索引  2019-12-07
  CUSTOM_NPC_END_INX = 999;                       //自定义NPC结束索引  2019-12-07
...
{$IFEND VER_176}
  ...
  SM_AUTOMOVE = 2015;        //   寻路  2019-12-14
  SM_MERCHANDISEMSG = 2016;  //   交易信息   2019-12-30；
  SM_SELLSECCESSITEM = 2017;  //    批量卖物品时成功卖出位
  ...
  SM_TEST = 65037;
  SM_THROW = 65069;
  SM_BAGDURACHANGE = 65070; // 2019-11-01
  ...
  RM_PASSWORDSTATUS = 11017;
  RM_SPELLFAIL = 11018;   //  2019-10-21   技能失败
  RM_BAGDURACHANGE = 11019; // 2019-11-01 改变背包内物品DURA
  RM_OPENSTORAGEVIEW = 11020; //  2019-12-19 可视化仓库消息
  ...
  CM_QUERYCD = 3503;
  CM_SPACEMOVE = 3504;                      //    增加寻路右键戴戒指飞  2019-12-23
  CM_GETMERCHANDISEMSG = 3505;              //    交易信息   2019-12-30；
  CM_SENDSELLITEMLIST = 3506;
  ...
  SM_SPELLFAIL = 20011;     //   2019-10-21  技能释放失败

  CM_SELLOFF = 4004;
  CM_BUYSELLOFFITEM = 4005;
  CM_GETSALEDETAILITEM = 4006;
  SM_IMAGEFILELISTCUSTOM = 20012; //新增自定义资源文件列表  2019-12-04
  SM_SAFEZONEEFFECTCUSTOM = 20013; //新增查询自定义安全区光圈特效  2019-12-04
  SM_NPCCUSTOM = 20014;           //新增自定义NPC效果 2019-12-07
  SM_OPENSTORAGEVIEW = 20015; //新增可视化仓库  2019-12-19
  SM_GETSTORAGEITEMLIST = 20016; //新增获取仓库列  2019-12-20
  CM_USERTAKEBACKSTORAGEITEMVIEW = 20017; //新增取仓库物品  2019-12-21
  CM_USERSTORAGEITEMVIEW = 20018;//新增存物品到仓库  2019-12-21
  TiShenItemIndex = 80;
...
type
  TStartPointRegion = record //异形安全区控制 2019-12-16
    nSizeCode: Integer; //对应SIZE代码
    rgn:HRGN; //异形区域
  end;
  pTStartPointRegion = ^TStartPointRegion;

  TStartPointCustom = record         //自定义安全区光圈特效  2019-12-04
    nEffectsType: Word;            //光圈特效类型
    nEffectsFileIndex: Word;      //特效文件索引 从ImageFileList.txt中读取
    nEffectsStartOffset: Word;    //特效起始贞
    nEffectsCountOffset: Word;    //播放贞数
    bEffectsBlendDraw: Boolean;   //是否透明绘制
    nEffectsSpeed: Integer;       //播放速度(毫秒)
  end;
  pTStartPointCustom = ^TStartPointCustom;

  TNpcCustom = record               //自定义NPC  2019-12-07
    nNpcCode: Word;               //Npc代码[CUSTOM_NPC_START_INX..CUSTOM_NPC_END_INX]
    nNpcDir: Byte;                //Npc方向[0..7] 与NpcCode做复合主键
    nResFileIndex: Word;          //资源文件索引 从ImageFileList.txt中读取
  ...
  pTNpcCustom = ^TNpcCustom;




  TLoginData = record      // 2019-10-14
    loginName: String[255];//登录器名称
    urlMaster: String[255];//主服务器列表 示例 http://aaa.com/liebiao.txt
    urlSlave: String[255];//备用服务器列表 示例 http://aaa.com/liebiao.txt
    urlUpdate: String[255];///更新服务器列表 示例  http://aaa.com/update.txt
    webSite: String[255];//官网地址
    urlGG: String[255];//公告URL
    urlKF: String[255];//联系客服
    urlZB: String[255];//装备
    skinOffset: Int64; //皮肤文件偏移量
    skinSize: Int64;//皮肤文件长度
    loginToolOffset: Int64; //登录器文件偏移量
    loginToolSize: Int64; //登录器文件长度
    mirOffset: Int64;//游戏客户端偏移量
    mirSize: Int64;//游戏客户端文件长度
    corePatchOffset: Int64;//游戏核心补丁包偏移量
    corePatchSize: Int64;//游戏核心补丁包长度
    OpenCount: byte;//游戏允许多开的数量
    ResourceRead: byte;//加载 1读取Wis标志 2读取Wil标志 4是读取Wzl标志
    MicroAddress: String[255]; //微端地址
    MicroPort: Word;//微端端口
    MicroMode: byte;//微端模式 0.纯微 1.轻端
    showCoreVersion: Boolean;//显示客户端程序内核版本信息
    CoreChkStr:String[255];//核心字符串，该字符串放在结构体最后做为核心特征码检验
  end;
  ...
  TSuiteIndex = array[0..High(Byte)] of Byte;
...
{$IFEND HIGHHP}
  ...
  TItemType = (t_aMake, t_dMake, t_dTakeOff, t_aSellOff, t_dSell, t_dDeal, t_dDrop, t_dStorage, t_pOnLogon, t_dUpgrade, t_dRepair, t_sDrop, t_ADrop, t_dCustomName, t_dHeroUse, t_dps, t_nsc, t_dTakeOn, t_MonDropItem, t_PickItem);
  ...
  {THumData = packed record
    ...
    VenationInfos: TVenationInfos;
    //wStatusTimeArrEx: TStatusTimeEx;    //0x54
    //sTemp: string[167 - SizeOf(TStatusTimeEx)];
...
type
  ...
  TItemLimit = record
    ...
    boNoScatter: Boolean;
    bodTakeOn: Boolean;
    boMonDropItem: Boolean; //怪物掉落触发  2019-11-23
    boPickItem:Boolean; //捡取触发  2019-11-23
  end;
  ...
  TShopItem = packed record //0x98
    ...
    wShape1: Word; //0X14
    wShape2: Word; //0X16
    wPresent: Byte; // 允许赠送 0.不允许赠送 1.允许赠送
    wMarketType: Byte; //商品销售方式 0.金币 1.元宝
    sExplain: string[127];
  end;
...
function APPRfeature(cfeature: string): Word;
function RACEfeature(cfeature: String): Byte;
function HAIRfeature(cfeature: string): Word;
function DRESSfeature(cfeature: string): Byte;
function WEAPONfeature(cfeature: string): Byte;
...
begin
  New(g_dwIPNeedInfo);
  for i := 1 to MAX_IPLEVEL do
  begin
    g_dwIPNeedInfo^[i].nPower := i * i + 10;
    t := Int64(200) * i * i * i + 13940 * i * i + 55330;
...
function WEAPONfeature(cfeature: string): Byte;
begin
  if Length(cfeature) = SizeOf(ThumFeature)then
     Result := PThumFeature(@cfeature[1]).Weapon
  else if Length(cfeature)  = SizeOf(TMonFeature) then
    Result := pTMonFeature(@cfeature[1]).Weapon
  else
    Result := 0;
...
begin
  Result := 0;
  if Length(cfeature) = SizeOf(TMonFeature) then
    Result := pTMonFeature(cfeature).wAppr;
...
begin
  if Length(cfeature) = SizeOf(ThumFeature) then
     Result := PThumFeature(cfeature).Hair
  else
    Result := 0;
  // Result := HiWord(cfeature);
end;
...
begin
  Result := 0;
  if length(cfeature) = sizeof(TMonFeature) then
     Result := pTMonFeature(cfeature).RaceImg
  else if length(cfeature) = sizeof(THUmFeature) then
     Result := pTHumFeature(cfeature).RaceImg;
end;
...
{$I '..\Common\Macros\VMPBM.inc'}
  ...
  if (lpszDosPath = nil) or (nlen = 0) then
  begin
    Exit;
  end;
  ...
  begin
    ...
    if QueryDosDevice(szDrive, szName, SizeOf(szName)) > 0 then
    begin
