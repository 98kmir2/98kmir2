...
uses
  SysUtils, Windows, Classes, VMProtectSDK;
...
begin
  ...
  for i := 0 to Length(FSMBiosTablesList) - 1 do
    if FSMBiosTablesList[i].Header.TableType = Ord(TableType) then
    begin
...
begin
  FSWbemLocator := CreateOleObject(VMProtectDecryptStringA('WbemScripting.SWbemLocator'));
  szTemp1 := VMProtectDecryptStringA('localhost');
  szTemp2 := VMProtectDecryptStringA('root\WMI');
  FWMIService := FSWbemLocator.ConnectServer(szTemp1, szTemp2, '', '');
  szTemp3 := VMProtectDecryptStringA('SELECT * FROM MSSmBios_RawSMBiosTables');
  szTemp4 := VMProtectDecryptStringA('WQL');
