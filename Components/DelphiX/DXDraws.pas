...
type
  ...
  public
    ...
    procedure TextOutA(X, Y: Integer; const Text: string; bk: Integer = 1);
  end;
...
begin
  if DirectDrawDrivers = nil then begin
...
begin
  if (FSurface.IDDSurface <> nil) and (FDC <> 0) then begin
    Handle := 0;
...
procedure TDirectDrawSurfaceCanvas.TextOutA(X, Y: Integer; const Text: string; bk: Integer);
begin
 //  OutputDebugString(pchar(Text));
  ClFunc.DxBoldTextOut(FSurface, X, Y, FSurface.Canvas.Font.Color, clBlack, Text, 0, bk);
end;
...
begin
  if Source = nil then
  ...
  else if Source is TDirectDrawSurface then begin
    if TDirectDrawSurface(Source).IDDSurface = nil then
      ...
      FDDraw.DXResult := FDDraw.IDraw.DuplicateSurface(TDirectDrawSurface(Source).IDDSurface,
        TempSurface);
      if FDDraw.DXResult = 0 then begin
...
{$ENDIF DrawHWAcc}
    ...
    if (SrcRect.Left > SrcRect.Right) or (SrcRect.Top > SrcRect.Bottom) then begin
      {  Mirror  }
      if ((X + Abs(SrcRect.Left - SrcRect.Right)) <= 0) or ((Y + Abs(SrcRect.Top - SrcRect.Bottom)) <= 0) then Exit;

      //g_DF.dwSize := SizeOf(g_DF);
      g_DF.dwDDFX := 0;

      if SrcRect.Left > SrcRect.Right then begin
        i := SrcRect.Left;
      ...
      if SrcRect.Top > SrcRect.Bottom then begin
        i := SrcRect.Top;
      ...
      if ClipRect2(DestRect, SrcRect, ClientRect, Source.ClientRect) then begin
        if g_DF.dwDDFX and DDBLTFX_MIRRORLEFTRIGHT <> 0 then begin
          i := SrcRect.Left;
          SrcRect.Left := Source.Width - SrcRect.Right;
          SrcRect.Right := Source.Width - i;
        end;

        if g_DF.dwDDFX and DDBLTFX_MIRRORUPDOWN <> 0 then begin
          i := SrcRect.Top;
      ...
      if ClipRect2(DestRect, SrcRect, ClientRect, Source.ClientRect) then begin
        if FHasClipper then begin
          ...
          if DXResult = DDERR_BLTFASTCANTCLIP then begin
...
{$ENDIF DrawHWAcc}
    if FHasClipper then begin
      ...
      if FStretchDrawClipper = nil then begin
...
begin
  if Source <> nil then begin
    ...
    if ISurface.GetClipper(OldClipper) = DD_OK then begin
      DF.dwSize := SizeOf(DF);
      DF.dwDDFX := 0;
      Blt(DestRect, SrcRect, BltFlags[Transparent], DF, Source);
    end else begin
...
begin
  ...
  if dxrDDSurfaceLock(ISurface, DestSurface) then begin
    try
      if dxrDDSurfaceLock2(Source.ISurface, Src_ddsd, SrcSurface) then begin
        try
          if DestSurface.ColorType = DXR_COLORTYPE_INDEXED then begin
...
begin
  ...
  with SrcRect do
    ...
    end else if (Right - Left = AWidth) and (Bottom - Top = AHeight) then begin
      with Canvas do try
    ...
    if Lock(PRect(nil)^, ddsd) then begin
      try
        case ddsd.ddpfPixelFormat.dwRGBBitCount of
          ...
          24: with PRGB(Integer(ddsd.lpSurface) + Y * ddsd.lPitch + X * 3)^ do begin
              R := Byte(Value);
              G := Byte(Value shr 8);
              B := Byte(Value shr 16);
            end;
...
begin
  ...
  with ddsd do begin
    dwHeight := AHeight;
    dwWidth := AWidth;
    if not FSystemMemory then FSystemMemory := True;
    dwSize := SizeOf(ddsd);
...
begin
  ...
  if BitCount = 16 then
    ...
    Color16 := aColor;
  if deltax > deltay then {// horizontal or vertical} begin
...
begin
  if Source is TDXDrawDisplay then begin
...
begin
  Result := nil;
  if FDXDraw.DDraw <> nil then begin
...
begin
  ...
  for i := 0 to Count - 1 do
    if (Modes[i].Width = Width) and (Modes[i].Height = Height) and (Modes[i].BitCount = BitCount) then begin
      Result := i;
      Exit;
    end;
end;
...
function TDXDrawDisplay.DynSetSize(AWidth, AHeight, ABitCount: Integer): Boolean;
  ...
  begin
    if (BitCount > 8) and (ABitCount > 8) then begin
      Result := True;
    end else begin
  ...
  begin
    ...
    for i := 0 to Count - 1 do
      with Modes[i] do begin
        if ((DWidth >= Width) and (DHeight >= Width) and
          ...
          ((not FFixedBitCount) and TestBitCount(BitCount, ABitCount)))) then begin
          DWidth := Width;
          DHeight := Height;
          DBitCount := BitCount;
          Flag := True;
    ...
    if Flag then begin
      if (DBitCount <> ABitCount) then begin
...
begin
  ...
  if SetSize(AWidth, AHeight, ABitCount) then begin
    Result := True;
    Exit;
  end;
  ...
  if FFixedRatio then
    if SetSize2(True) then begin
      Result := True;
      Exit;
    end;

  {  The change is unconditionally attempted.  }
  if SetSize2(False) then begin
    Result := True;
    Exit;
  end;
end;
...
type
  ...
  begin
    ...
    if idoOptimizeDisplayMode in rec.Options then begin
  ...
  var
    ...
    begin
      ...
      for j := Low(Bits) to High(Bits) do begin
  ...
  begin
    ...
    try
      if (DDraw.DriverCaps.dwCaps and DDCAPS_3D <> 0) and
        (DDraw.DriverCaps.ddsCaps.dwCaps and DDSCAPS_TEXTURE <> 0) then begin
...
begin
  ...
  {  Driver selection   }
  if idoSelectDriver in Options then begin
    rec.Flag := False;
    rec.Options := Options;
    rec.Driver := @Driver;
  ...
  if rec.Flag and (idoOptimizeDisplayMode in Options) then begin
    if (rec.DeviceDesc.dwDeviceRenderBitDepth and BPPToDDBD(rec.BitCount)) = 0 then begin
...
type
  ...
  begin
    ...
    if ZBufferBitDepth <> 0 then begin
      with ddsd do begin
        dwSize := SizeOf(ddsd);
      ...
      if ZBuffer.CreateSurface(ddsd) then begin
        if Surface.ISurface.AddAttachedSurface(ZBuffer.ISurface) <> DD_OK then begin
          ZBuffer.Free;
          ZBuffer := nil;
          Exit;
        end;
...
var
  ...
  begin
    ...
    if idoZBuffer in Options then begin
...
begin
  try
    ...
    if D3D3.CreateDevice(D3DDeviceGUID, Surface.ISurface4, D3DDevice3, nil) <> D3D_OK then begin
      SupportHardware := False;
      InitDevice;
    ...
    if idoRetainedMode in Options then begin
      NowOptions := NowOptions + [idoRetainedMode];
{$IFDEF D3DRM}
      if D3DRM = nil then begin
      ...
      if Surface.BitCount = 8 then begin
...
begin
  ...
  try
    ...
    if D3D7.CreateDevice(D3DDeviceGUID, Surface.ISurface7, D3DDevice7) <> D3D_OK then begin
      SupportHardware := False;
      InitDevice;
...
begin
  ...
  if FDXDraw.FOptions * [doFullScreen, doSystemMemory, do3D, doHardware] = [doFullScreen, do3D, doHardware] then begin
    AOptions := [];
    with FDXDraw do begin
...
begin
  ...
  with FDXDraw do begin
    ...
    if doDirectX7Mode in FOptions then begin
...
begin
  ...
  if doStretch in FDXDraw.NowOptions then begin
    Dest := Bounds(pt.X, pt.Y, FDXDraw.Width, FDXDraw.Height);
  end else begin
    if doCenter in FDXDraw.NowOptions then begin
...
begin
  ...
  if not FDXDraw.FSurface.CreateSurface(ddsd) then begin
...
begin
  ...
  try
    ...
    if FDXDraw.FCalledDoInitializeSurface then begin
...
begin
  case Message.Msg of
    WM_WINDOWPOSCHANGED: begin
        if TWMWindowPosChanged(Message).WindowPos^.Flags and SWP_SHOWWINDOW <> 0 then begin
...
begin
  if Assigned(FOnWindowMove) then FOnWindowMove(Self);
end;
...
begin
  if FInternalInitialized then begin
    ...
    try
      try
        try
          if FCalledDoInitializeSurface then begin
      ...
      SetOptions(FOptions);
      if FDXDrawDriver <> nil then FDXDrawDriver.Free;
      FDXDrawDriver := nil;
      FUpdating := False;
    end;
  end;
...
begin
  if Initialized and (not FUpdating) then begin
...
begin
  ...
  Result := False;
  if Initialized and (not FUpdating) and (Surface.IDDSurface <> nil) and (Primary.IDDSurface <> nil) then begin
  ...
  try
    ...
    try
      ...
      try
        ...
        {  Set display mode.  }
        if doFullScreen in FNowOptions then begin
...
begin
{$IFDEF D3DRM}
  if FInitialized and (do3D in FNowOptions) and (doRetainedMode in FNowOptions) then begin
    asm FInit end;
    FViewport.Clear;
...
begin
  if Initialized and (not FUpdating) then begin
    FUpdating := True;
    try
      if TDXDrawDriver(FDXDrawDriver).Restore then begin
...
begin
  ...
  G := GetGValue(colorfrom);
  B := GetBValue(colorfrom);
  for i := R to 255 do begin
    Surface.Fill(RGB(i, G, B));
    Flip;
  end;
  for i := G to 255 do begin
    Surface.Fill(RGB(255, i, B));
    Flip;
  end;
  for i := B to 255 do begin
...
var
  i                         : Integer;
begin
  if shadefrom < shadeto then begin
    for i := shadefrom to shadeto do begin
      Surface.Fill(RGB(i, i, i));
      Flip;
    end;
  end
  else begin
    for i := shadefrom downto shadeto do begin
      Surface.Fill(RGB(i, i, i));
      Flip;
    end;
  end;
...
var
  Entries                   : TPaletteEntries;
begin
  if Initialized and (Palette <> nil) then begin
...
begin
  if ((ASurfaceWidth <> SurfaceWidth) or (ASurfaceHeight <> SurfaceHeight)) and (not FUpdating) then begin
    if Initialized then begin
      try
...
begin
  ...
  if Initialized and (not FUpdating) and (Surface.IDDSurface <> nil) and (Primary.IDDSurface <> nil) then begin //blue
    if (Primary.ISurface.IsLost = DDERR_SURFACELOST) or
      (Surface.ISurface.IsLost = DDERR_SURFACELOST) then begin
      if Assigned(FD2D) and Assigned(FD2D.FD2DTexture) then FD2D.FD2DTexture.D2DPruneAllTextures; //<-Add Mr.Kawasaki
      Restore;
      Result := (Primary.ISurface.IsLost = DD_OK) and (Surface.ISurface.IsLost = DD_OK);
    end else
...
begin
  if Initialized and (doWaitVBlank in FNowOptions) then begin
...
begin
  ...
  try
    ...
    if not FSurface.CreateSurface(ddsd) then begin
      ddsd.ddsCaps.dwCaps := ddsd.ddsCaps.dwCaps and (not DDSCAPS_VIDEOMEMORY) or DDSCAPS_SYSTEMMEMORY;
    ...
    if doDirectX7Mode in FDXDraw.NowOptions then begin
      InitializeDirect3D7(FSurface, FZBuffer, FD3D7, FD3DDevice7, AOptions);
    end else begin
      InitializeDirect3D(FSurface, FZBuffer, FD3D, FD3D2, FD3D3, FD3DDevice, FD3DDevice2, FD3DDevice3,
...
begin
{$IFDEF D3DRM}
  if FInitialized and (toRetainedMode in FNowOptions) then begin
    asm FInit end;
    FViewport.Clear;
    FViewport.Render(FScene);
    FD3DRMDevice.Update;
...
begin
  if (ASurfaceWidth <> SurfaceWidth) or (ASurfaceHeight <> SurfaceHeight) then begin
    FSurfaceWidth := ASurfaceWidth;
    FSurfaceHeight := ASurfaceHeight;
...
begin
  case NotifyType of
    dxntDestroying: begin
        DXDraw := nil;
      end;
    dxntInitializing: begin
        if (FDXDraw.FOptions * [do3D, doFullScreen] = [doFullScreen])
          and (FOptions * [toSystemMemory, toSelectDriver] = [toSelectDriver]) then begin
          AOptions := [];
          with FDXDraw do begin
            if doHardware in Options then AOptions := AOptions + [idoHardware];
            if doRetainedMode in Options then AOptions := AOptions + [idoRetainedMode];
...
begin
  ...
  if FDXDraw is TCustomDXDraw then begin
    with (FDXDraw as TCustomDXDraw) do begin
...
{$IFDEF DX3D_deprecated}
    if FDXDraw is TCustomDX3D then begin
      with (FDXDraw as TDX3D) do begin
...
{$IFDEF DX3D_deprecated}
  else if FDXDraw is TCustomDX3D then begin
...
procedure TDirect3DTexture.Restore;
  ...
  begin
    ...
    if not tex.FEnumFormatFlag then begin
      ...
      if (tex.FBitCount <= 8) and (ddsd.ddpfPixelFormat.dwRGBBitCount >= tex.FBitCount) and
        ...
        (ddsd.ddpfPixelFormat.dwFlags and DDPF_RGB <> 0) then begin
...
{$ENDIF};
    ...
    try
      ...
      {  Make palette.  }
      if ddsd.ddpfPixelFormat.dwFlags and DDPF_PALETTEINDEXED8 <> 0 then begin
        PaletteCaps := DDPCAPS_8BIT or DDPCAPS_ALLOW256;
        if FBitCount = 24 then
          CreateHalftonePalette(3, 3, 2);
      end else if ddsd.ddpfPixelFormat.dwFlags and DDPF_PALETTEINDEXED4 <> 0 then begin
        PaletteCaps := DDPCAPS_4BIT;
        if FBitCount = 24 then
          CreateHalftonePalette(1, 2, 1);
      end else if ddsd.ddpfPixelFormat.dwFlags and DDPF_PALETTEINDEXED1 <> 0 then begin
        PaletteCaps := DDPCAPS_1BIT;
        if FBitCount = 24 then begin
      ...
      with TempSurface.Canvas do begin
...
begin
  ...
  if FSrcImage is TDXTextureImage then
  ...
  else if FSrcImage is TGraphic then begin
    ...
    if FSrcImage is TPicture then begin
      FSrcImage := TDIB.Create;
      try
...
procedure TDirect3DTexture2.Initialize;
  ...
  begin
    ...
    if tex.FEnumTextureFormatFlag then begin
      if lpDDPixFmt.dwFlags and DDPF_PALETTEINDEXED <> 0 then begin
        {  Bit count check  }
        ...
        if lpDDPixFmt.dwFlags and DDPF_RGB <> 0 then begin
          ...
          if (rgb_alpha > 0) and (tex.FTextureFormat.ddpfPixelFormat.dwFlags and DDPF_ALPHAPIXELS = 0) and
            (lpDDPixFmt.dwFlags and DDPF_ALPHAPIXELS <> 0) then begin
            UseThisFormat;
            Exit;
          end;
          ...
          {  Bit count check  }
          if tex.FTextureFormat.ddpfPixelFormat.dwFlags and DDPF_PALETTEINDEXED = 0 then begin
...
begin
  Finalize;
  try
    if FDXDraw.D3DDevice7 <> nil then begin
    ...
    if FImage <> nil then begin
      ...
      if FD3DDevDesc.dpcTriCaps.dwTextureCaps and D3DPTEXTURECAPS_POW2 <> 0 then begin
      ...
      if FD3DDevDesc.dpcTriCaps.dwTextureCaps and D3DPTEXTURECAPS_SQUAREONLY <> 0 then begin
    ...
    {  Palette making  }
    if (FImage <> nil) and (FTextureFormat.ddpfPixelFormat.dwFlags and DDPF_PALETTEINDEXED <> 0) then begin
...
begin
  ...
  if (FSurface <> nil) and FTransparent and (FD3DDevDesc.dpcTriCaps.dwTextureCaps and D3DPTEXTURECAPS_TRANSPARENCY <> 0) then begin
    FillChar(ck, SizeOf(ck), 0);
    if FSurface.SurfaceDesc.ddpfPixelFormat.dwFlags and DDPF_PALETTEINDEXED <> 0 then begin
      if FTransparentColor shr 24 = $01 then begin
        {  Palette index  }
      ...
      if (FImage <> nil) and (FImage.ImageType = DXTextureImageType_PaletteIndexedColor) and (FTransparentColor shr 24 = $01) then begin
        {  Palette index  }
        ...
        if FTransparentColor shr 24 = $00 then begin
          {  RGB value  }
          ck.dwColorSpaceLowValue :=
...
const
  ...
  begin
    ddsd.dwSize := SizeOf(ddsd);
    if Dest.Lock(nil, ddsd, DDLOCK_WAIT, 0) = 0 then begin
      try
        if (SrcImage.idx_index.Mask = DWORD(1 shl ddsd.ddpfPixelFormat.dwRGBBitCount) - 1) and (SrcImage.idx_alpha.Mask = 0) and
          (SrcImage.BitCount = Integer(ddsd.ddpfPixelFormat.dwRGBBitCount)) and (not SrcImage.PackedPixelOrder) then begin
  ...
  begin
    ...
    if Dest.Lock(nil, ddsd, DDLOCK_WAIT, 0) = 0 then begin
      try
        ...
        if SrcImage.idx_alpha.Mask <> 0 then begin
          for Y := 0 to ddsd.dwHeight - 1 do
    ...
    if Dest.Lock(nil, ddsd, DDLOCK_WAIT, 0) = 0 then begin
      try
        ...
        if (dest_red_fmt.Mask = SrcImage.rgb_red.Mask) and (dest_green_fmt.Mask = SrcImage.rgb_green.Mask) and
          ...
          (Integer(ddsd.ddpfPixelFormat.dwRGBBitCount) = SrcImage.BitCount) and (not SrcImage.PackedPixelOrder) then begin
          for Y := 0 to ddsd.dwHeight - 1 do
            Move(SrcImage.ScanLine[Y]^, Pointer(Integer(ddsd.lpSurface) + ddsd.lPitch * Y)^, (Integer(ddsd.dwWidth) * SrcImage.BitCount + 7) div 8);
        end else
          if SrcImage.rgb_alpha.Mask <> 0 then begin
            for Y := 0 to ddsd.dwHeight - 1 do
              for X := 0 to ddsd.dwWidth - 1 do begin
                c := SrcImage.Pixels[X, Y];
...
begin
  ...
  if SurfaceDesc.ddpfPixelFormat.dwFlags and DDPF_PALETTEINDEXED <> 0 then begin
    case SrcImage.ImageType of
      DXTextureImageType_PaletteIndexedColor: LoadTexture_IndexToIndex;
      DXTextureImageType_RGBColor: ;
    end;
  end else if SurfaceDesc.ddpfPixelFormat.dwFlags and DDPF_RGB <> 0 then begin
    case SrcImage.ImageType of
...
begin
  ...
  if indexed then begin
    ...
    if Result._rshift < 0 then begin
...
begin
  if _DXTextureImageLoadFuncList = nil then begin
...
begin
  j := 0;
  for i := 0 to SubImageCount - 1 do
    if SubImages[i].ImageGroupType = GroupTypeID then begin
...
begin
  if ImageType = DXTextureImageType_PaletteIndexedColor then begin
...
begin
  ...
  P := Stream.Position;
  for i := 0 to DXTextureImageLoadFuncList.Count - 1 do begin
    Stream.Position := P;
    try
  ...
  begin
    while True do begin
      ...
      case BlockHeader.ID of
        DXTextureImageFileBlockID_EndGroup: begin
            {  End of group  }
            Break;
          end;
        DXTextureImageFileBlockID_StartGroup: begin
            {  Beginning of group  }
            Stream.ReadBuffer(Header_StartGroup, SizeOf(Header_StartGroup));
            case Header_StartGroup.CategoryType of
              DXTextureImageFileCategoryType_Image: begin
                  {  Image group  }
        ...
        DXTextureImageFileBlockID_Image_Format: begin
            ...
            if Header_Image_Format.ImageType = DXTextureImageType_PaletteIndexedColor then begin
              {  INDEX IMAGE  }
            ...
            end else if Header_Image_Format.ImageType = DXTextureImageType_RGBColor then begin
              {  RGB IMAGE  }
        ...
        DXTextureImageFileBlockID_Image_Name: begin
            {  Name reading  }
        ...
        DXTextureImageFileBlockID_Image_GroupInfo: begin
        ...
        DXTextureImageFileBlockID_Image_TransparentColor: begin
...
begin
  ...
  while True do begin
    ...
    case BlockHeader.ID of
      DXTextureImageFileBlockID_EndFile: begin
          {  End of file  }
          Break;
        end;
      DXTextureImageFileBlockID_StartGroup: begin
      ...
      if Image.Size > 0 then begin
        ...
        try
          ...
          case Image.FileCompressType of
            DXTextureImageFileCompressType_None: begin
                Header_Image_PixelData.Compress := DXTextureImageCompress_None;
              end;
{$IFDEF DXTextureImage_UseZLIB}
            DXTextureImageFileCompressType_ZLIB: begin
...
begin
  ...
  {  Bit mask reading  }
  if BI.biCompression = BI_BITFIELDS then begin
    Stream.ReadBuffer(PixelFormat, SizeOf(PixelFormat));
  end else begin
    if BI.biBitCount = 16 then begin
      PixelFormat.RBitMask := $7C00;
      PixelFormat.GBitMask := $03E0;
      PixelFormat.BBitMask := $001F;
    end else if (BI.biBitCount = 24) or (BI.biBitCount = 32) then begin
  ...
  {  DIB making  }
  if BI.biHeight < 0 then begin
    BI.biHeight := -BI.biHeight;
    TopDown := True;
  end else
    TopDown := False;

  if BI.biBitCount in [1, 4, 8] then begin
...
begin
  ...
  for Channel := Low(Channel) to High(Channel) do
    if Channel in FHasChannels then begin
      Flag := False;
      for i := 0 to HasImageChannelListCount - 1 do
        if HasImageChannelList[i].Image = FHasChannelImages[Channel].Image then begin
  ...
  if Image.ImageType = DXTextureImageType_PaletteIndexedColor then begin
    {  Index color  }
    for Y := 0 to Image.Height - 1 do
      for X := 0 to Image.Width - 1 do begin
        c := 0;

        for i := 0 to HasImageChannelListCount - 1 do begin
    ...
    if Image.ImageType = DXTextureImageType_RGBColor then begin
      {  RGB color  }
      for Y := 0 to Image.Height - 1 do
        for X := 0 to Image.Width - 1 do begin
          c := 0;
...
begin
  ...
  (FParamsFormat.MipmapCount = 0) then begin
  ...
  for Channel := Low(TDXTImageChannel) to High(TDXTImageChannel) do
    if (FHasChannelImages[Channel].Image <> nil) and (FHasChannelImages[Channel].Image.ImageType = DXTextureImageType_PaletteIndexedColor) then begin
      FParamsFormat.idx_palette := FHasChannelImages[Channel].Image.idx_palette;
      Break;
    end;

  if FParamsFormat.ImageType = DXTextureImageType_PaletteIndexedColor then begin
    {  Index channel }
    if rgbRed in FHasChannels then begin
    ...
    {  Alpha channel  }
    if rgbAlpha in FHasChannels then begin
    ...
    {  B channel }
    if rgbBlue in FHasChannels then begin
    ...
    {  G channel }
    if rgbGreen in FHasChannels then begin
  ...
  {  Transparent color  }
  if (FParamsFormat.ImageType = DXTextureImageType_RGBColor) and (FParamsFormat.TransparentColor shr 24 = $01) then begin
...
begin
  ...
  j := 0;
  for Q := rgbRed to rgbAlpha do begin
    if Q in CheckChannelUsed then begin
      Inc(j);
      Channel := Q;
      if not (Channel in FHasChannels) then begin
        ...
        if CheckChannelChanged <> '' then begin
  ...
  {  Processing of each  }
  if ChannelParamName <> [] then begin
    {  Picture load  }
    Image := nil;
    {pokud je image uz nahrany tj. stejneho jmena, pokracuj dale}
    for j := 0 to FHasImageList.Count - 1 do
      if AnsiCompareFileName(TDXTextureImage(FHasImageList[j]).ImageName, FStrImageFileName) = 0 then begin
...
{$ENDIF}
    ...
    for Channel := Low(Channel) to High(Channel) do
      if Channel in ChannelParamName then begin
        if ChannelBitCount[Channel] >= 0 then
...
begin
  ...
  try
    ...
    if FParamsFormat.ImageType = DXTextureImageType_RGBColor then begin
      BuildImage(Result);
      {  Picture information store here  }
      CurWidth := FParamsFormat.Width;
      CurHeight := FParamsFormat.Height;
      for i := 0 to FParamsFormat.MipmapCount - 1 do begin
...
begin
  if Source is TPictureCollectionItem then begin
...
begin
  if FSurfaceList.Count = 0 then begin
...
begin
  if FInitialized and (PatternIndex >= 0) and (PatternIndex < FPatterns.Count) then begin
{$IFDEF DrawHWAcc}
...
begin
  if FInitialized and (PatternIndex >= 0) and (PatternIndex < FPatterns.Count) then
    with TPictureCollectionItemPattern(FPatterns.Items[PatternIndex]) do begin
...
begin
  ...
  if (Width = FPicture.Width) and (Height = FPicture.Height) then begin
    ...
    for Y := 0 to BlockYCount - 1 do
      for X := 0 to BlockXCount - 1 do begin
    ...
    for Y := 0 to (FPicture.Height + SkipHeight) div Height2 - 1 do
      for X := 0 to (FPicture.Width + SkipWidth) div Width2 - 1 do begin
        X2 := X * Width2;
        Y2 := Y * Height2;
        with TPictureCollectionItemPattern.Create(FPatterns) do begin
...
begin
  if Value <> FTransparentColor then begin
    FTransparentColor := Value;
    for i := 0 to FSurfaceList.Count - 1 do begin
      try
...
var
    ...
    for i := 0 to 255 do
      if UseColorTable[i] then
        if DWORD(ColorTable[i]) = DWORD(Col) then
          Exit;
    for i := 0 to 255 do
      if not UseColorTable[i] then begin
        SetColor(i, Col);
        Exit;
      end;
  end;
  ...
  begin
    if Graphic.Palette <> 0 then begin
...
begin
  ...
  for i := 0 to Count - 1 do
    if Items[i].Picture.Graphic <> nil then begin
...
begin
  ...
  try
    ...
    if SurfaceDesc.ddsCaps.dwCaps and DDSCAPS_FLIP <> 0 then begin
...
begin
  FOverlayRect := Value;
  if (FSurface <> nil) and FVisible then begin
    ...
    if (FDDraw.DriverCaps.dwCaps and DDCAPS_OVERLAYSTRETCH <> 0) and
      (FDDraw.DriverCaps.dwMinOverlayStretch <> 0) and (XScaleRatio < Integer(FDDraw.DriverCaps.dwMinOverlayStretch)) then begin
    ...
    if (FDDraw.DriverCaps.dwCaps and DDCAPS_OVERLAYSTRETCH <> 0) and
      (FDDraw.DriverCaps.dwMaxOverlayStretch <> 0) and (XScaleRatio > Integer(FDDraw.DriverCaps.dwMaxOverlayStretch)) then begin
    ...
    if (FDDraw.DriverCaps.dwCaps and DDCAPS_OVERLAYSTRETCH <> 0) and
      (FDDraw.DriverCaps.dwMinOverlayStretch <> 0) and (YScaleRatio < Integer(FDDraw.DriverCaps.dwMinOverlayStretch)) then begin
    ...
    if (FDDraw.DriverCaps.dwCaps and DDCAPS_OVERLAYSTRETCH <> 0) and
      (FDDraw.DriverCaps.dwMaxOverlayStretch <> 0) and (YScaleRatio > Integer(FDDraw.DriverCaps.dwMaxOverlayStretch)) then begin
    ...
    if DestRect.Left < 0 then begin
      SrcRect.Left := -DestRect.Left * 1000 div XScaleRatio;
      DestRect.Left := 0;
    end;

    if DestRect.Right > Integer(FTargetSurface.SurfaceDesc.dwWidth) then begin
    ...
    if DestRect.Bottom > Integer(FTargetSurface.SurfaceDesc.dwHeight) then begin
    ...
    if (FDDraw.DriverCaps.dwCaps and DDCAPS_ALIGNBOUNDARYSRC <> 0) and (FDDraw.DriverCaps.dwAlignBoundarySrc <> 0) then begin
    ...
    if (FDDraw.DriverCaps.dwCaps and DDCAPS_ALIGNSIZESRC <> 0) and (FDDraw.DriverCaps.dwAlignSizeSrc <> 0) then begin
    ...
    if (FDDraw.DriverCaps.dwCaps and DDCAPS_ALIGNBOUNDARYDEST <> 0) and (FDDraw.DriverCaps.dwAlignBoundaryDest <> 0) then begin
    ...
    if (FDDraw.DriverCaps.dwCaps and DDCAPS_ALIGNSIZEDEST <> 0) and (FDDraw.DriverCaps.dwAlignSizeDest <> 0) then begin
    ...
    {  Color key setting  }
    if FDDraw.DriverCaps.dwCKeyCaps and DDCKEYCAPS_DESTOVERLAY <> 0 then begin
...
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (AComponent = FDXImageList) then begin
    FDXImageList := nil;
  end;
end;                                    {Notification}
...
begin
  ...
  for Loop := 1 to Length(UpperText) do begin
...
begin
  FFont := Value;
  if FDXImageList <> nil then begin
    FFontIndex := FDXImageList.Items.IndexOf(FFont); // Find font once...
    Offset := FDXImageList.Items[FFontIndex].PatternWidth;
...
begin
  ...
  Offset := FDXImageList.Items[FFontIndex].PatternWidth;
  Loop := 1;
  while (Loop <= Length(Text)) do begin
...
begin
  ...
  XLoop := 0;
  Loop := 1;
  while (Loop <= Length(txt)) do begin
    if FUseEnterChar then begin
      if txt[Loop] = FEnterCharacter[1] then begin
        Inc(FCalculatedEnters);
        Inc(Loop);
      end;
      if txt[Loop] = FEnterCharacter[2] then begin
        Inc(FCalculatedEnters);
...
//  FDDraw.D3DDevice7.SetTextureStageState(0, D3DTSS_MINFILTER, DWord(Ord(FD2DTextureFilter)+1));
  ...
  if (Image.PatternHeight <> 0) or (Image.PatternWidth <> 0) then begin
    ...
    diffX := 0.5;
    if Image.PatternCount = 1 then diffX := 0;
...
begin
  ...
  if (do3D in FDDraw.Options)
    and (doDirectX7Mode in FDDraw.Options)
    and (doHardware in FDDraw.Options) then begin
...
begin
  ...
  if PWidth = 0 then PWidth := Image.Width;
  PHeight := Image.PatternHeight;
  if PHeight = 0 then PHeight := Image.Height;
...
begin
  with FBlitRec do begin
    ...
    if FAnimLooped then begin
      ...
      if Round(FAnimPos) >= FAnimCount then begin
        FAnimPos := FAnimCount - 1;
        FAnimSpeed := 0;
      end;
      if FAnimPos < 0 then begin
        FAnimPos := 0;
        FAnimSpeed := 0;
      end;
    end;
      ...
      if FEngine.Traces.Items[i].FBlit <> Self then
        ...
        with FEngine.Traces.Items[i] do
          ...
          if OverlapRect(Bounds(Round(FBlit.WorldX), Round(FBlit.WorldY),
            FBlit.Width, FBlit.Height), Bounds(Round(WorldX), Round(WorldY), Width, Height)) then begin
...
function TBlit.GetBlitAt(X, Y: Integer): TBlit;
  ...
  begin
    if Blit.Visible and PointInRect(Point(Round(X), Round(Y)),
      Bounds(Round(Blit.X), Round(Blit.Y), Blit.Width, Blit.Width)) then begin
