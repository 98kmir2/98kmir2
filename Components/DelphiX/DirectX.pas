...
{$ENDIF}
  ...
  TDDSurfaceDesc_DX6 = packed record
    dwSize: DWORD;                      // 00 size of the TDDSurfaceDesc structure
    dwFlags: DWORD;                     // 04 determines what fields are valid
    dwHeight: DWORD;                    // 08 height of surface to be created
    dwWidth: DWORD;                     // 0c width of input surface
    case integer of
      0: (
        dwLinearSize: DWORD;            // unused at the moment
        );
      1: (
        lPitch: LongInt;                // 10 distance to start of next line (return value only)
        dwBackBufferCount: DWORD;       // 14 number of back buffers requested
        case integer of
          0: (
            dwMipMapCount: DWORD;       // 18 number of mip-map levels requested
            dwAlphaBitDepth: DWORD;     // 1c depth of alpha buffer requested
            dwReserved: DWORD;          // 20 reserved
            lpSurface: Pointer;         // 24 pointer to the associated surface memory
            ddckCKDestOverlay: TDDColorKey; // color key for destination overlay use
            ddckCKDestBlt: TDDColorKey; // color key for destination blt use
            ddckCKSrcOverlay: TDDColorKey; // color key for source overlay use
            ddckCKSrcBlt: TDDColorKey;  // color key for source blt use
...
begin
  for x := 0 to 255 do
    with _c_dfDIKeyboard_Objects[x] do begin
      PGUID := @GUID_Key;
      dwOfs := x;
      dwFlags := 0;
...
begin
  ...
  for x := 0 to 2 do begin              // 3 more blocks of X axis..Sliders
...
procedure LoadDSetup;
  ...
  begin
    ...
    RegType := REG_SZ;
    try
      if (RegOpenKeyEx(Hive, PChar(KeyName), 0, KEY_READ, EnvKey) = ERROR_SUCCESS) then begin
        try
