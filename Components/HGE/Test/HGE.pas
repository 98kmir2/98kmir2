...
uses
  Classes, Windows, {$IFDEF HGE_DX8}DirectXGraphics{$ELSE}Direct3D9{$ENDIF}, Bass, OpenJpeg, Graphics,
{$IFDEF HGE_DX8}D3DX81mo, {$ELSE}D3DX9, {$ENDIF}
  Forms;
...
const
  ...
  VERTEX_BUFFER_SIZE        = 4000;     //GetAvailableTextureMemory
...
type
  IHGE = interface
    ...
    function Texture_Create(const Width, Height: Integer; Format: TD3DFormat = D3DFMT_A8R8G8B8): ITexture;
...
begin
  ...
  Rect.Right := 0;
{$IFDEF HGE_DX9}
  if not Assigned(FTextSprite) then Exit;
  ...
  if Succeeded(FHandle.GetLevelDesc(0, Desc)) then begin
    if (X >= 0) and (X < Desc.Width) and (Y >= 0) and (Y < Desc.Height) then begin
      if (Desc.Format = D3DFMT_A8R8G8B8) or (Desc.Format = D3DFMT_X8R8G8B8) then begin
        ...
        if Succeeded(FHandle.LockRect(0, Rect, PRec, Flags)) then begin
          try
            Result := PLongint(LongInt(Rect.pBits) + Y * Rect.Pitch + X * 2)^;
          finally
            Unlock;
          end;
        end;
  ...
  if Succeeded(FHandle.GetLevelDesc(0, Desc)) then begin
    if (X >= 0) and (X < Desc.Width) and (Y >= 0) and (Y < Desc.Height) then begin
      if (Desc.Format = D3DFMT_A8R8G8B8) or (Desc.Format = D3DFMT_X8R8G8B8) then begin
        ...
        if Succeeded(FHandle.LockRect(0, Rect, PRec, Flags)) then begin
          try
            PLongint(LongInt(Rect.pBits) + Y * Rect.Pitch + X * 2)^ := Value;
          finally
            Unlock;
          end;
        end;
...
begin
  ...
  if Assigned(FDepth) then
    PHGE.FD3DDevice.CreateDepthStencilSurface(FWidth, FHeight,
      D3DFMT_D16, D3DMULTISAMPLE_NONE, {$IFDEF HGE_DX8}FDepth{$ELSE}0, False, FDepth, nil{$ENDIF});
end;
...
begin
  ...
  if Assigned(FIB) then begin
    FD3DDevice.SetIndices(nil{$IFDEF HGE_DX8}, 0{$ENDIF});
    FIB := nil;
  end;
  ...
  if Assigned(FVB) then begin
    ...
    FD3DDevice.SetStreamSource(0, nil, {$IFDEF HGE_DX9}0, {$ENDIF}SizeOf(THGEVertex));
    FVB := nil;
  end;
...
{$ENDIF}
  ...
  NModes := FD3D.GetAdapterModeCount(D3DADAPTER_DEFAULT{$IFDEF HGE_DX9}, Mode.Format{$ENDIF});
  for I := 0 to NModes - 1 do begin
    FD3D.EnumAdapterModes(D3DADAPTER_DEFAULT, {$IFDEF HGE_DX9}Mode.Format, {$ENDIF}I, Mode);
...
{$ENDIF}
  ...
  System_Log('AvailableTextureMemory: %dM',
    [FD3DDevice.GetAvailableTextureMem div 1048576]);

  {FD3DDevice.GetDeviceCaps(D3DCaps);
...
begin
  ...
  try
    for I := 0 to FTargets.Count - 1 do begin
      Target := TTarget(FTargets[I]);   //blue
      Target.Restore;
    end;
  except
  end;
...
begin
  ...
  if (Target <> FCurTarget) then begin
    ...
    if (Failed(FD3DDevice.SetRenderTarget({$IFDEF HGE_DX8}Surf, Depth{$ELSE}0, Surf{$ENDIF}))) then begin
  ...
  FVB.Lock(0, 0, {$IFDEF HGE_DX8}PByte{$ELSE}Pointer{$ENDIF}(FVertArray), 0);
  Result := True;
end;
...
begin
  ...
  FD3DDevice.GetRenderTarget({$IFDEF HGE_DX9}0, {$ENDIF}FScreenSurf);
  ...
  if (Failed(FD3DDevice.CreateVertexBuffer(VERTEX_BUFFER_SIZE * SizeOf(THGEVertex),
    D3DUSAGE_WRITEONLY, D3DFVF_HGEVERTEX, D3DPOOL_DEFAULT, FVB{$IFDEF HGE_DX9}, 0{$ENDIF}))) then begin
...
{$ENDIF}
  ...
  if (Failed(FD3DDevice.CreateIndexBuffer(VERTEX_BUFFER_SIZE * 6 div 4 * SizeOf(Word),
    D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_DEFAULT, FIB{$IFDEF HGE_DX9}, nil{$ENDIF}))) then begin
    PostError('Can''t create D3D index buffer');
    Exit;
  end;

  N := 0;
  if (Failed(FIB.Lock(0, 0, {$IFDEF HGE_DX8}PByte{$ELSE}Pointer{$ENDIF}(PIndices), 0))) then begin
  ...
  FIB.Unlock;
  FD3DDevice.SetIndices(FIB{$IFDEF HGE_DX8}, 0{$ENDIF});
  ...
  if Assigned(FVertArray) then begin
    ...
    if (FPrim <> 0) then begin
      case FCurPrimType of
        HGEPRIM_QUADS:
          FD3DDevice.DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, {$IFDEF HGE_DX9}0, {$ENDIF}FPrim shl 2, 0, FPrim shl 1);
        HGEPRIM_TRIPLES:
...
begin
  ...
  if FNoBorder then begin
    //Width := FScreenWidth + GetSystemMetrics(SM_CXSIZEFRAME) * 2;
    //Height := FScreenHeight + GetSystemMetrics(SM_CYSIZEFRAME) * 2;
    ...
    //Width := FScreenWidth + GetSystemMetrics(SM_CXSIZEFRAME) * 2;
    //Height := FScreenHeight + GetSystemMetrics(SM_CYSIZEFRAME) * 2 + GetSystemMetrics(SM_CYCAPTION);
    //FStyleW := WS_POPUP or WS_CAPTION or WS_SYSMENU or WS_MINIMIZEBOX or WS_MAXIMIZEBOX or WS_SIZEBOX or WS_VISIBLE; //WS_OVERLAPPED | WS_SYSMENU | WS_MINIMIZEBOX;
    Width := FScreenWidth + GetSystemMetrics(SM_CXFIXEDFRAME) * 2;
...
begin
  ...
  if Assigned(FD3DDevice) then begin
    if Succeeded(FD3DDevice.GetBackBuffer({$IFDEF HGE_DX9}0, {$ENDIF}0, D3DBACKBUFFER_TYPE_MONO, Surf)) then begin
      D3DXSaveSurfaceToFile(PChar(ShotName), D3DXIFF_PNG, Surf, nil, nil);
    end;
  end;
end;
...
begin
  ...
  if (ZBuffer) then begin
    if (Failed(FD3DDevice.CreateDepthStencilSurface(Desc.Width, Desc.Height,
      D3DFMT_D16, D3DMULTISAMPLE_NONE, {$IFDEF HGE_DX8}Depth{$ELSE}0, False, Depth, nil{$ENDIF}))) then begin
...
function THGEImpl.Texture_Create(const Width, Height: Integer; Format: TD3DFormat = D3DFMT_A8R8G8B8): ITexture;
var
...
begin
  if (Failed(D3DXCreateTexture(FD3DDevice, Width, Height,
    ...
    PTex))) then begin
    PostError('Can''t create texture 1');
    Result := nil
  end else begin
...
var
  pVB                       : {$IFDEF HGE_DX8}PByte{$ELSE}Pointer{$ENDIF};
begin
  FD3DDevice.SetVertexShader({$IFDEF HGE_DX8}D3DFVF_HGEVERTEX{$ELSE}nil{$ENDIF});
  FVB.Lock(0, SizeOf(THGEVertex) * numVertices, pVB, D3DLOCK_DISCARD);
  Move(pVertices^, pVB^, SizeOf(THGEVertex) * numVertices);
  FVB.Unlock;
  FD3DDevice.SetStreamSource(0, FVB, {$IFDEF HGE_DX9}0, {$ENDIF}SizeOf(THGEVertex));
end;
...
begin
  ...
  FD3DDevice.SetGammaRamp({$IFDEF HGE_DX9}0, {$ENDIF}0, FGammaRamp);
end;
