...
type
  ...
  public
    ...
    function LoadCurve(Filename: String):Boolean;
...
Function TNURBSCurveEX.LoadCurve(Filename: String):Boolean;
var
  i: Integer;
  Data:IResource;
  Size:LongWord;
...
begin
  ...
  for i := 0 to CPCount - 1  do
    begin
      ControlPoints[i].X := PD^;
      Inc(PD);
      ControlPoints[i].Y := PD^;
      Inc(PD);
    end;
  Data:=nil;
  FittingCurveReady := False;
  UpdateKnots;
  LoadBakeCurve(Filename + BAKE_EXT);
  Result:=True;
end;
