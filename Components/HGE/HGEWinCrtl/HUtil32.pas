...
uses
	Classes, windows,SysUtils,StrUtils, WinTypes, WinProcs, Graphics, Messages, Dialogs,Math,TlHelp32;
...
function  Str_ToInt64 (Str: string; def: int64): Int64;
...
function _MIN (n1, n2: int64): integer;
function _MAX (n1, n2: integer): integer;
function GetItemShape:integer;
function CheckItemBlockName(s:String):boolean;
function IPStrToLong(IP:String):DWord;
function IsIPaddr(IP:String):boolean;
function IntToSex(i:integer):String;
function IntToJob(i:integer):String;
function GetCodeMsgSize(d:double):Integer;
function IntToStr2(i:Integer):String;
function BoolToIntStr(b:Boolean):string;
function BoolToCStr(b:Boolean):String;
function IsChinese(Str: string): Boolean;
procedure GetDirFileList(sPath: string; FileList: TStringList);
function SpaceStr(s:String;Len:Integer;nAlign:Integer):String;
function DateToChStr(D:TDateTime;boTime:boolean=false):String;
function ReplaceStr(sMsg, sStr, sText: String): String;
function GetToken(s:String;splitchar:string;var p:integer):String;
function EndProcess(ExeName:string):Integer;
implementation
...
begin
  ...
  i:=posex(splitchar,s,p);
  if i<=0 then i:=length(s)+1;
  result:=copy(s,p,i-p);
  p:=i+length(splitchar);
end;
...
begin
  ...
  result:=format('%d年%d月%d日',[yy,mm,dd]);
  if boTime then begin
     decodetime(d,hh,nn,ss,ms);
     result:=result+' '+ format('%d时%d分%d秒',[hh,nn,ss]);
  end;
end;
...
begin
  if sPath[Length(sPath)] <> '\' then
    sPath := sPath + '\';
  if FindFirst(sPath + '*.txt', faAnyFile, SearchRec) = 0 then begin
    while true do begin
...
function IntToJob(i:integer):String;
begin
  if i=0 then result:='战士' else if i=1 then result:='法师'
  else if i=2 then result:='道士' else result:='未知';
end;

function IntToSex(i:integer):String;
begin
  if i=0 then result:='男' else result:='女';
end;
...
begin
  try
    ...
    s3:=getToken(ip,'.',i);
    s4:=getToken(ip,'.',i);
    result:=MakeLong(MakeWord(strtoint(s1),strtoint(s2)),MakeWord(strtoint(s3),strtoint(s4)));
  except
    result:=0;
  end;
end;
...
begin
  ...
  windows.AdjustTokenPrivileges(hToken, FALSE, tkp, sizeof(tkpNewButIgnored),tkpNewButIgnored, ReturnLength);
  e:=GetLastError();
  if (e <> ERROR_SUCCESS) then begin
    exit;
  end;
...
begin
  ...
  while Integer(ContinueLoop) <> 0 do begin
    if (UpperCase(FProcessEntry32.szExeFile) = UpperCase(ExeName)) then begin
