...
type
  IHGEParticleSystem = interface
    ...
    function GetParticles(nInex: Integer): PHGEParticle;
...
constructor Create(pRes: Pointer; nSize: LongWord; const Sprite: IHGESprite); overload;
...
begin
  ...
  FAge := -2;
  FBoundingBox.Clear;
  if Sprite <> nil then Sprite.SetBlendMode(FBlendMode);
end;

constructor THGEParticleSystem.Create(pRes: Pointer; nSize: LongWord; const Sprite: IHGESprite);
var
  P                         : PByte;
  SprCount                  : Integer;
begin
...
function THGEParticleSystem.GetBlendMode: Integer;
begin
  Result := FBlendMode;
end;
...
function THGEParticleSystem.GetParticles(nInex: Integer): PHGEParticle;
begin
  Result := nil;
  if (nInex >= 0) and (nInex < FParticlesAlive) then
    Result := @FParticles[nInex];
end;
