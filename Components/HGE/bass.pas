...
uses
  SysUtils, Classes, DLLLoader;

var
  IsInitOK: Boolean;
  BassDllLoader: TDLLLoader = nil;
...
begin
  ...
  BassDllRes := TResourceStream.Create(Hinstance, 'ResData', 'Bass_Dll');
  if BassDllRes = nil then Exit;

  BassDllRes.Position := 0;

  BassDllLoader := TDLLLoader.Create;
  if not BassDllLoader.Load(BassDllRes) then
  begin
    BassDllRes.Free;
    Result := False;
    Exit;
