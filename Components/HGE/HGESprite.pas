...
type
  ...
  public
    property Quad: THGEQuad read FQuad;
...
begin
  ...
  if Assigned(Tex) then begin
    ...
    if TW = 0 then TW := FHGE.Texture_GetWidth(Tex, True);
    TH := FHGE.Texture_GetHeight(Tex);
    if TH = 0 then TH := FHGE.Texture_GetHeight(Tex, True);
  end else begin
    TW := 1.0;
    TH := 1.0;
  end;
