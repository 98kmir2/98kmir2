...
{Modify By Chenlong 2003.8 Install on Delphi 7 }
unit D7ScktComp;
...
type
  ...
  public
    ...
    property Addr: TSockAddrIn read FAddr write FAddr;  //1234
...
begin
  ...
  FLookupState := lsIdle;
  if not (asConnect in FAsyncStyles) then begin
    FConnected := FSocket <> INVALID_SOCKET;
    ...
    if FSendStream <> nil then begin
      ...
      while True do begin
        ...
        if AmountInBuf > 0 then begin
          ...
          else if FSendStream.Position = FSendStream.Size then begin
            DropStream;
            Break;
          end;
        end else begin
...
begin
  if Value <> FAsyncStyles then begin
    FAsyncStyles := Value;
...
begin
  ...
  try
    ...
    if (Count = -1) then
      ...
      if ioctlsocket(FSocket, FIONREAD, iCount) = 0 then begin
...
var
  ErrorCode                 : Integer;
begin
  if Message.LookupHandle = FLookupHandle then begin
    FLookupHandle := 0;
...
begin
  ...
  if Assigned(FServerWinSocket) then begin
...
begin
  FListLock.Enter;
  try
    if FActiveThreads.IndexOf(AThread) < 0 then begin
...
begin
  ...
  if getsockopt(INVALID_SOCKET, SOL_SOCKET, SO_OPENTYPE, PChar(@OldOpenType),
    Len) = 0 then try
    if FServerType = stThreadBlocking then begin
    ...
    //messageBox(0, PChar(IntToStr(Addr.sin_addr.S_addr)), '', 0);
    if ClientWinSocket <> INVALID_SOCKET then begin
      ClientSocket := GetClientSocket(ClientWinSocket);
      ClientSocket.Addr := Addr;    //1234
      if Assigned(FOnSocketEvent) then  begin
  ...
  try
    ...
    try
      ...
      while FActiveThreads.Count > 0 do
        with TServerClientThread(FActiveThreads.Last) do begin
...
begin
  ...
  try
    for I := 0 to FActiveThreads.Count - 1 do
      if TServerClientThread(FActiveThreads[I]).ClientSocket = nil then begin
...
begin
  ...
  try
    for I := 0 to FActiveThreads.Count - 1 do
      if TServerClientThread(FActiveThreads[I]).ClientSocket = ClientSocket then begin
        Result := FActiveThreads[I];
        Break;
      end;
  finally
...
begin
  if CompareText(Value, FAddress) <> 0 then begin
...
begin
  if CompareText(Value, FHost) <> 0 then begin
    if not (csLoading in ComponentState) and FActive then
      raise ESocketError.CreateRes(@sCantChangeWhileActive);
    FHost := Value;
  end;
...
begin
  if CompareText(Value, FService) <> 0 then begin
    if not (csLoading in ComponentState) and FActive then
      raise ESocketError.CreateRes(@sCantChangeWhileActive);
    FService := Value;
  end;
...
begin
  if (Value <> FClientSocket.Connected) and not (csDesigning in ComponentState) then begin
...
begin
  if (Value <> FServerSocket.Connected) and not (csDesigning in ComponentState) then begin
