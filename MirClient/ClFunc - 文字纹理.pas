...
begin
  ...
  try
    ...
    begin
      ...
      begin
        WordColor := lpBuffer^;
        TempMap^ := WordColor;              //  fputc(GetBlue(word), fp);         // 蓝
        Inc(TempMap);
        Inc(lpBuffer);
      end;
      Dec(lpBuffer,nPitch);
...
begin
  ...
  try
    ...
    //DebugOutStr(Format('AWidth:%d, AHeight:%d', [nWidth, nHeight]));
        ...
        begin
          Hbrsh := windows.CreateSolidBrush(RGB(0, 0, 0));                                  // 画背景
          windows.FillRect(HHDC, Rect(0, 0, w, h), Hbrsh);
          DeleteObject(Hbrsh);

          SelectObject(HHDC, HHBitmap);
          SetTextColor(HHDC, clRed);
          SelectObject(HHDC, frmMain.FFont.Handle);                                         //设文字颜色为
          SetBkColor(HHDC, RGB(255, 255, 255));                                                   //设背景颜色为黑色
