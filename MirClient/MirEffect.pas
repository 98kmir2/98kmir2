...
uses
  ...
  Grobal2, DXDraws, cliUtil, ClFunc, HUtil32, DWinCtl, WIL;
...
begin
  ...
  if m_boActive then
    g_SndMgr.PlaySound('wav\OpenBox.wav');
  Result := m_boActive;
end;
...
begin
  Result := True;
  if m_boActive and not m_boRareBoxShow and (GetTickCount - m_nStepTime > LongWord(m_nNextFrameTime)) then
  begin
    m_nStepTime := GetTickCount;
    Inc(m_nCurFrame);
    if m_nCurFrame > m_nStart + m_nFrame - 1 then
    begin
