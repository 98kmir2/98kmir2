...
uses
  ...
  Grobal2, DXDraws, cliUtil, magiceff, WIL, ClFunc, GList, Actor, MShare;
...
var
  ...
  begin
    ...
    if (cx > MAXX * 3) or (cy > MAXY * 3) then
      Exit;
...
begin
  ...
  for i := g_APPathList.count - 1 downto 0 do
  begin
...
begin
  FreeTree(); //内存泄露???
  if g_APQueue <> nil then
  begin
    if g_APQueue.Next <> nil then
      Dispose(g_APQueue.Next);
    g_APQueue.Next := nil;
    if g_APQueue.Node <> nil then
      Dispose(g_APQueue.Node);
    g_APQueue.Node := nil;
    Dispose(g_APQueue);
    g_APQueue := nil;
  end;
...
begin
  ...
  begin //红没有回城
    ...
    begin
      if (g_ItemArr[i].S.Name <> '') and (g_ItemArr[i].S.AC > 0) and (g_ItemArr[i].S.StdMode = 0) then
      begin
        has := True;
        Break;
      end;
  ...
  begin //蓝没有回城
    ...
    for i := 0 to MAXBAGITEMCL - 1 do
    begin
      if (g_ItemArr[i].S.Name <> '') and (g_ItemArr[i].S.MAC > 0) and (g_ItemArr[i].S.StdMode = 0) then
      begin
        has := True;
        Break;
      end;
  ...
  begin
    ...
    for i := 0 to MAXBAGITEMCL - 1 do
    begin
      if (g_ItemArr[i].S.StdMode = 25) and (g_ItemArr[i].S.Name <> '') and (Pos('符', g_ItemArr[i].S.Name) > 0) then
      begin
        has := True;
        Break;
      end;
    ...
    begin
      ...
      for i := 0 to MAXBAGITEMCL - 1 do
      begin
        if (g_ItemArr[i].S.StdMode = 25) and (g_ItemArr[i].S.Name <> '') and (Pos('药', g_ItemArr[i].S.Name) > 0) then
        begin
          has := True;
          Break;
        end;
...
begin
  ...
  begin
    if (not g_APTagget.m_boDeath) and (g_APTagget.m_nHiterCode = g_MySelf.m_nRecogId) and (g_APTagget.m_boVisible) and (not g_APTagget.m_boDelActor) then
    begin
...
begin
  ...
  begin
    ...
    if g_boPickUpAll or d.boPickUp then
    begin //如果拾取过滤，则判断是否过滤
      dx := GetDis(d.X, d.Y, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY); //获取距离，选择最近的
      if (dx < Result) and (dx <> 0) then
      begin
        g_AutoPicupItem := d;
        Result := dx;
      end;
...
begin
  ...
  //  exit;
  bPcaketfull := False;
  if IsBackToSafeZone(Mobdistance) then
  begin
    Result := 0;
    Exit;
  end
  ...
  begin
    ...
    begin
      if (((abs(g_APLastPoint.X - g_MySelf.m_nCurrX) >= overdisc) or (abs(g_APLastPoint.Y - g_MySelf.m_nCurrY) >= overdisc))) and
        ((abs(g_APMapPath[g_APStep].X - g_MySelf.m_nCurrX) >= overdisc) or (abs(g_APMapPath[g_APStep].Y - g_MySelf.m_nCurrY) >= overdisc)) then
      begin
        g_nOverAPZone := 14;
        Result := 4;
        Exit;
    ...
    begin
      if ((abs(g_APMapPath[g_APStep].X - g_MySelf.m_nCurrX) >= overdisc) or (abs(g_APMapPath[g_APStep].Y - g_MySelf.m_nCurrY) >= overdisc)) then
      begin
        g_nOverAPZone := 14;
        Result := 4;
        Exit;
  ...
  if ((GetTickCount - g_dwSearchEnemyTick) > 4000) or (((GetTickCount - g_dwSearchEnemyTick) > 300) and ((g_APTagget = nil))) then
  begin
  ...
  //两者都没有发现
  if (ItemDistance = 100000) and ((Mobdistance = 100000) or (Mobdistance = 0)) then
  begin //没有发现怪物或物品，随机走
    Result := 3;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      Actor := TActor(m_ActorList[i]);
      if (abs(Actor.m_nCurrX - g_MySelf.m_nCurrX) < 6) or (abs(Actor.m_nCurrY - g_MySelf.m_nCurrY) < 6) then
      begin
        if IsProperTarget(Actor) then
        begin
          Inc(Result);
...
begin
  ...
  begin
    ...
    begin
      Actor := TActor(m_ActorList[i]);
      if (abs(Actor.m_nCurrX - g_MySelf.m_nCurrX) < 5) or (abs(Actor.m_nCurrY - g_MySelf.m_nCurrY) < 5) then
      begin
        if IsProperTarget(Actor) then
        begin
          Inc(Result);
...
begin
  ...
  begin
    ...
    begin
      Actor := TActor(m_ActorList[i]);
      if (abs(Actor.m_nCurrX - g_MySelf.m_nCurrX) < 8) or (abs(Actor.m_nCurrY - g_MySelf.m_nCurrY) < 8) then
      begin
...
begin
  ...
  begin
    if (g_MySelf.m_HeroObject.m_nHeroEnergyType <> 2) and (g_MySelf.m_HeroObject.m_nHeroEnergy > 0) and (g_MySelf.m_HeroObject.m_nHeroEnergy >= g_MySelf.m_HeroObject.m_nMaxHeroEnergy) then
    begin
      if GetTickCount - g_dwLatestJoinAttackTick <= 3000 then
        Exit;
      pcm := nil;
      case g_MySelf.m_btJob of
        0:
          begin
            ...
            if (pcm <> nil) and frmMain.CanNextAction and frmMain.ServerAcceptNextAction and frmMain.CanNextHit then
            begin
      ...
      if (pcm <> nil) and frmMain.CanNextAction and frmMain.ServerAcceptNextAction then
      begin
        g_dwLatestSpellTick := GetTickCount;
        g_dwLastMoveTick := GetTickCount;
        Result := True;
...
begin
  ...
  case g_MySelf.m_btJob of
    0:
      begin
        //
        if g_SeriesSkillReady then
        begin
          frmMain.SendFireSerieSkill();
          //Result := True;
        end;
        if g_gcTec[4] and (g_MySelf.m_nState and $00100000 = 0) and CanNextSpell() then
        begin
        ...
        if XPATTACK() or frmMain.AttackTarget(g_APTagget) then
        begin
          Result := True;
          Exit;
        end;
    ...
    1:
      begin
        if g_MySelf.m_Abil.Level < 7 then
        begin
        ...
        if g_SeriesSkillReady and (g_MagicLockActor <> nil) and (not g_MagicLockActor.m_boDeath) then
        begin
          frmMain.SendFireSerieSkill();
        end;
        if XPATTACK() then
        ...
        begin
          if (nAbsX <= g_nMagicRange) and (nAbsY <= g_nMagicRange) then
          begin
            Result := True;
            ...
            begin
              ...
              if CanNextSpell() then
              begin

                if (g_MagicArr[0][22] <> nil) then
                begin
                  if TargetCount3(g_APTagget) >= 15 then
                  begin
                    ...
                    if EventMan.GetEvent(nx, ny, ET_FIRE) = nil then
                    begin
                ...
                if g_nTagCount >= 2 then
                begin
                  if Random(7) > 1 then
                  begin
                    if (g_MagicArr[0][58] <> nil) and (Random(8) > 1) then
                    begin
                      MagicKey := 58;
                    end
                    else if (g_MagicArr[0][33] <> nil) then
                      MagicKey := 33;
                  end
                  else if (g_MagicArr[0][47] <> nil) then
                    MagicKey := 47;
        ...
        begin
          ...
          begin //目标近身
            ...
            begin
              ...
              if (nTag >= 4) and (g_MagicArr[0][8] <> nil) then
              begin //比较勉强的抗拒...一般选择逃避
                Result := True;
                m_dwTargetFocusTick := GetTickCount();
                if AutoUseMagic(8, g_MySelf) then
                begin
                  if m_btMagPassTh <= 0 then
          ...
          while True do
          begin
            if g_PlayScene.CanWalk(nx, ny) then
              Break;
            Inc(tdir);
            tdir := tdir mod 8;
            GetBackPosition(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, nx, ny);
          ...
          begin
            ...
            if g_PlayScene.CanWalk(nTX, nTY) {Map.CanMove(nTX, nTY)} then
            begin
    ...
    2:
      begin ////////////////////////////////////////
        ...
        if (g_UseItems[U_ARMRINGL].S.StdMode = 25) and
          ...
          (Pos('药', g_UseItems[U_ARMRINGL].S.Name) > 0) then
        begin
          Inc(n);
        end;
        if n = 0 then
        begin
          ...
          begin
            if (g_ItemArr[i].S.NeedIdentify < 4) and
              ...
              (Pos('药', g_ItemArr[i].S.Name) > 0) then
            begin
              Inc(n);
              Break;
            end;
        ...
        if n = 0 then
        begin
          if GetTickCount - g_hinttick1 > 60 * 1000 then
          begin
        ...
        if (g_UseItems[U_ARMRINGL].S.StdMode = 25) and
          (g_UseItems[U_ARMRINGL].S.Shape <> 6) and
          (Pos('符', g_UseItems[U_ARMRINGL].S.Name) > 0) then
        begin
          Inc(m);
        end;
        if m = 0 then
        begin
          ...
          begin
            if (g_ItemArr[i].S.NeedIdentify < 4) and
              (g_ItemArr[i].S.StdMode = 25) and
              (g_ItemArr[i].S.Shape <> 6) and
              (Pos('符', g_ItemArr[i].S.Name) > 0) then
            begin
              Inc(m);
              Break;
            end;
        ...
        if m = 0 then
        begin
          if GetTickCount - g_hinttick2 > 60 * 1000 then
          begin
        ...
        if (GetTickCount - m_dwRecallTick) > (1000 * 6) then
        begin //设置比较大时间,以便其他攻击...
          m_dwRecallTick := GetTickCount();
          if (g_MySelf.m_SlaveObject.count = 0) and (m > 0) then
          begin
            MagicKey := 0;
        ...
        if (GetTickCount - m_dwSpellTick) > (1000 * 5) then
        begin //状态类魔法...
          m_dwSpellTick := GetTickCount();

          // MAGDEFENCEUP
          if (g_MagicArr[0][14] <> nil) and (m > 0) then
          begin
            if (g_MySelf.m_nState and $00200000 = 0) then
            begin
              Result := True;
            ...
            if g_MySelf.m_HeroObject <> nil then
            begin
              if (g_MySelf.m_HeroObject.m_nState and $00200000) = 0 then
              begin
                Result := True;
          ...
          //  Double DefenceUp
          if (g_MagicArr[0][15] <> nil) and (m > 0) then
          begin
            if (g_MySelf.m_nState and $00400000) = 0 then
            begin
              Result := True;
            ...
            if g_MySelf.m_HeroObject <> nil then
            begin
              if (g_MySelf.m_HeroObject.m_nState and $00400000 = 0) then
              begin
                Result := True;
          ...
          //  Healling
          if (g_MagicArr[0][2] <> nil) then
          begin
            if (Round((g_MySelf.m_Abil.HP / g_MySelf.m_Abil.MaxHP) * 100) < 85) then
            begin
              Result := True;
            ...
            if g_MySelf.m_HeroObject <> nil then
            begin
              if (g_MySelf.m_HeroObject.m_Abil.HP <> 0) and (abs(g_MySelf.m_nCurrX - g_MySelf.m_HeroObject.m_nCurrX + 2) <= g_nMagicRange) and (abs(g_MySelf.m_nCurrY - g_MySelf.m_HeroObject.m_nCurrY + 2) <= g_nMagicRange) then
              begin
                if (Round((g_MySelf.m_HeroObject.m_Abil.HP / g_MySelf.m_HeroObject.m_Abil.MaxHP) * 100) <= 80) then
                begin
                  Result := True;
            ...
            if g_MySelf.m_SlaveObject.count > 0 then
            begin
              for i := 0 to g_MySelf.m_SlaveObject.count - 1 do
              begin
                if TActor(g_MySelf.m_SlaveObject[i]).m_boDeath then
                  Continue;
                if (TActor(g_MySelf.m_SlaveObject[i]).m_Abil.HP <> 0) and (abs(g_MySelf.m_nCurrX - TActor(g_MySelf.m_SlaveObject[i]).m_nCurrX + 2) <= g_nMagicRange) and (abs(g_MySelf.m_nCurrY - TActor(g_MySelf.m_SlaveObject[i]).m_nCurrY + 2) <= g_nMagicRange) then
                begin
                  if (Round((TActor(g_MySelf.m_SlaveObject[i]).m_Abil.HP / TActor(g_MySelf.m_SlaveObject[i]).m_Abil.MaxHP) * 100) <= 80) then
                  begin
                    Result := True;
        ...
        if (g_MySelf.m_Abil.Level < 18) or (g_MagicArr[0][13] = nil) or ((n = 0) and (m = 0)) then
        begin
          CCCC:
          if (GetTickCount - m_dwSpellTick) > (3000) then
          begin
            m_dwSpellTick := GetTickCount();
            ...
            begin
              ...
              begin
                if (g_MySelf.m_HeroObject.m_Abil.HP <> 0) and (abs(g_MySelf.m_nCurrX - g_MySelf.m_HeroObject.m_nCurrX + 2) <= g_nMagicRange) and (abs(g_MySelf.m_nCurrY - g_MySelf.m_HeroObject.m_nCurrY + 2) <= g_nMagicRange) then
                begin
                  if (Round((g_MySelf.m_HeroObject.m_Abil.HP / g_MySelf.m_HeroObject.m_Abil.MaxHP) * 100) < 85) then
                  begin
                    if AutoUseMagic(2, g_MySelf.m_HeroObject) then
                      Exit;
                  end;
              ...
              begin
                if TActor(g_MySelf.m_SlaveObject[i]).m_boDeath then
                  Continue;
                if (abs(g_MySelf.m_nCurrX - TActor(g_MySelf.m_SlaveObject[i]).m_nCurrX + 2) <= g_nMagicRange) and (abs(g_MySelf.m_nCurrY - TActor(g_MySelf.m_SlaveObject[i]).m_nCurrY + 2) <= g_nMagicRange) then
                begin
                  if (TActor(g_MySelf.m_SlaveObject[i]).m_Abil.HP <> 0) and (Round((TActor(g_MySelf.m_SlaveObject[i]).m_Abil.HP / TActor(g_MySelf.m_SlaveObject[i]).m_Abil.MaxHP) * 100) < 85) then
                  begin
                    if AutoUseMagic(2, TActor(g_MySelf.m_SlaveObject[i])) then
                      Exit;
                  end;
        ...
        begin //需要快速检测类...
          ...
          begin
            ...
            begin
              ...
              begin
                //  DoubluSC
                if (g_MagicArr[0][50] <> nil) then
                begin
                  if GetTickCount - m_dwDoubluSCTick > 90 * 1000 then
                  begin
                ...
                if GetTickCount - m_dwPoisonTick > 3500 then
                begin
                  m_dwPoisonTick := GetTickCount;
                  if (g_MagicArr[0][6] <> nil) then
                  begin
                    if (g_APTagget.m_nState and $80000000 = 0) or (g_APTagget.m_nState and $40000000 = 0) then
                    begin
                      m_dwTargetFocusTick := GetTickCount();
                      MagicKey := 6;
                      goto BBBB;
                  ...
                  if (g_MagicArr[0][18] <> nil) and (g_MySelf.m_nState and $00800000 = 0) and (Random(4) = 0) then
                  begin
                    if (TargetCount2(g_MySelf) >= 7) then
                    begin
                      m_dwTargetFocusTick := GetTickCount();
                      MagicKey := 18;
                      goto BBBB;
                ...
                if (g_MagicArr[0][13] <> nil) or (g_MagicArr[0][57] <> nil) then
                begin
                  if (g_MagicArr[0][57] <> nil) and (((Round((g_MySelf.m_Abil.HP / g_MySelf.m_Abil.MaxHP) * 100) < 80) and (Random(100 - Round((g_MySelf.m_Abil.HP / g_MySelf.m_Abil.MaxHP) * 100)) > 5)) or (Random(10) > 6)) then
                  begin
                    m_dwTargetFocusTick := GetTickCount();
                    MagicKey := 57;
                    goto BBBB;
                  end;
                  if (g_MagicArr[0][13] <> nil) then
                  begin
                    m_dwTargetFocusTick := GetTickCount();
                    MagicKey := 13;
                    goto BBBB;
        ...
        begin
          ...
          begin //目标近身
            ...
            begin
              ...
              if (g_MagicArr[0][48] <> nil) and (nTag >= 3) then
              begin //有力的抗拒...不同于法师逃避
                Result := True;
                m_dwTargetFocusTick := GetTickCount();
                if AutoUseMagic(48, g_MySelf) then
                begin
                  if m_btMagPassTh <= 0 then
                    Inc(m_btMagPassTh, 1 + Random(2));
                  Exit;
