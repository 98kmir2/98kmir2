      ...
      begin
        ...
        begin
          case mag of
            ...
            8, 27, 33, 35, 37..39, 41, 43, 44, 45..48:
              begin
                wimg := g_WMagic2Images;
                if mag in [0..MAXEFFECT - 1] then
                  idx := g_EffectBase[mag];
  ...
  case Mtype of
    mtFly, mtBujaukGroundEffect, mtExploBujauk:
      begin
        ...
        if id = 44 then
          ExplosionFrame := 16;
        if (id = 81) or (id = 82) or (id = 83) then
        begin //if (id - 81 - 3) < 0 then begin
          bt80 := 1;
          Repetition := True;
          if id = 81 then
          begin
...
begin
  ...
  if fFirstShift and (m_nMagEffectNo in [0075, 108, 111]) then
  begin
    fFirstShift := False;
  ...
  else if (frame > 0) and (GetTickCount - steptime > LongWord(NextFrameTime)) then
  begin
    steptime := GetTickCount;
    Inc(curframe);
    if curframe > start + frame - 1 then
  ...
  begin
    ...
    begin
      ...
      if (nStdX <> 0) and (nStdY <> 0) and (m_nMagEffectNo in [0075, 108, 111]) then
      begin
      ...
      if m_nMagEffectNo in [0075, 108, 111] then
      begin
      ...
      if ((abs(targetx - FlyX) <= 15) and (abs(targety - FlyY) <= 15)) or OverThrough(OldDir16, passdir16) then
      begin
        boCrash := True;
      end
      else
    ...
    if boCrash and ((TargetActor <> nil) or (m_nMagEffectNo in [0075, 108, 111])) then
    begin
      FixedEffect := True;
      start := 0;
...
begin
  ...
  begin
    ...
    if not FixedEffect then
    begin
      if (ServerMagicId = 81) or (ServerMagicId = 82) or (ServerMagicId = 83) {or (magnumber = 47)} then
      begin
      ...
      begin
        ...
        if m_nMagEffectNo = 116 then
        begin
          if (MagOwner <> nil) and (curframe = 5) then
          begin
            g_PlayScene.CXYfromMouseXY(targetx, targety, mx, my);
...
begin
  ...
  begin
    ...
    begin
      if m_nMagEffectNo = MAGIC_FOX_FIRE2 then
      begin
        NextFrameTime := 100;
        img := 1320 + curframe;
      ...
      else if m_nMagEffectNo = MAGIC_FOX_CURSE then
      begin
        NextFrameTime := 100;
        img := 1330 + curframe;
        d := g_WMon33Img.GetCachedImage(img, px, py);
      ...
      else if m_nMagEffectNo = 108 then
      begin
      ...
      else if m_nMagEffectNo = 105 {112} then
      begin
      ...
      begin
        ...
        if (m_nMagEffectNo = 0075) and (curframe = 1) then
        begin
...
begin
  ...
  if not FixedEffect then
  begin
    if ((abs(targetx - FlyX) <= 15) and (abs(targety - FlyY) <= 15)) then
    begin
      FixedEffect := True;
      start := 0;
      frame := ExplosionFrame;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if MagicNumber = 74 then
      begin
        GetEffectBase(MagicNumber - 1, 0, ImgLib, img);
        img := img + curframe;
      end
      else if MagicNumber = 11 then
      begin
