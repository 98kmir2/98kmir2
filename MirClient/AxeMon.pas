...
uses
  ...
  Grobal2, DXDraws, cliUtil, ClFunc, magiceff, HumanActor, Actor, clEvent;
...
type
  ...
  TGhostShipMonster = class(TActor)
    ...
    FLighting: Boolean;
  private
   //  procedure AttackEff;     Ë½ÓÐ³ÉÔ±º¯ÊýÎ´µ÷ÓÃ   2019-10-07 18:01:33
  protected
  ...
  case m_nCurrentAction of
    ...
    SM_WALK, SM_BACKSTEP:
      begin
        ...
        if m_btRace in [118, 119] then
        begin
          if (Random(3000) mod 3) = 1 then
          begin
            if m_btRace = 118 then
              g_SndMgr.PlaySound(3451);
            if m_btRace = 119 then
              g_SndMgr.PlaySound(3461);
          end;
          m_boUseEffect := True;
          m_dwEffectstarttime := GetTickCount;
        end;
...
begin
  if (m_btRace = 117) and m_boDeath then
  begin
    m_BodySurface := nil;
    Exit;
  end;
  ...
  case m_wAppearance of
    ...
    707: if (m_nCurrentAction = SM_DIGUP) then
      begin
        m_boUseEffect := True;
    ...
    704: if (m_nCurrentAction = SM_NOWDEATH) then
      begin
        m_boUseEffect := True;
        EffectSurface := g_WMagic8Images.GetCachedImage(1970 + m_nCurrentFrame - m_nStartFrame, ax, ay);
      end;
    706: if (m_nCurrentAction = SM_NOWDEATH) then
      begin
        m_boUseEffect := True;
        EffectSurface := g_WMagic8Images.GetCachedImage(1980 + m_nCurrentFrame - m_nStartFrame, ax, ay);
      end;
    708: if (m_nCurrentAction = SM_NOWDEATH) then
      begin
        m_boUseEffect := True;
        EffectSurface := g_WMagic8Images.GetCachedImage(1990 + m_nCurrentFrame - m_nStartFrame, ax, ay);
      end;
...
begin
  ...
  begin
    if Integer(GetTickCount - m_dwSmoothMoveTime) > 200 then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else if m_wAppearance = 706 then
      begin
      ...
      else if m_wAppearance = 708 then
      begin
  ...
  case m_nCurrentAction of
    SM_HIT:
      begin
        ...
        m_dwWarModeTime := GetTickCount;
        Shift(m_btDir, 0, 0, 1);
        if not (m_btRace in [27, 28, 111]) then
        begin
          m_boUseEffect := True;
          m_neffectframe := m_nStartFrame;
          m_nEffectStart := m_nStartFrame;
    ...
    SM_LIGHTING, SM_LIGHTING_1:
      begin
        ...
        else if (m_btRace in [27, 28]) then
        begin
          m_boUseEffect := True;
          m_neffectframe := m_nStartFrame;
          m_nEffectStart := m_nStartFrame;
          m_nEffectEnd := m_nEndFrame;
          if m_btRace = 28 then
            m_nEffectEnd := m_nEndFrame - 4;
          m_dwEffectstarttime := GetTickCount;
          m_dwEffectframetime := m_dwFrameTime;
        end
        else if (m_btRace in [113, 114]) then
        begin
          m_nStartFrame := pm.ActAttack.start + m_btDir * (pm.ActAttack.frame + pm.ActAttack.skip);
          m_nEndFrame := m_nStartFrame + pm.ActAttack.frame - 1;
          m_dwFrameTime := pm.ActAttack.ftime;
          if m_btRace = 113 then
          begin
            m_boUseEffect := True;
        ...
        else if (m_btRace = 117) and (m_nCurrentAction = SM_LIGHTING_1) then
        begin
          m_boUseEffect := True;
        ...
        else if m_btRace in [118, 119] then
        begin
          m_boUseEffect := True;
          m_boUseMagic := False;
          m_dwEffectstarttime := GetTickCount;
  ...
  begin
    case m_btRace of
      ...
      72:
        begin
          if m_nCurrentAction = SM_HIT then
            m_DrawEffect := g_WMon21Img.GetCachedImage(3490 + (m_btDir * 10) + m_neffectframe - m_nEffectStart, ax, ay);
          if m_nCurrentAction = SM_LIGHTING then
            m_DrawEffect := g_WMon21Img.GetCachedImage(3580 + m_neffectframe - m_nEffectStart, ax, ay);
        end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if m_nCurrentAction = SM_LIGHTING then
      begin
        if (m_btRace = 117) and (m_nCurrentFrame - m_nStartFrame = 1) then
        begin
        ...
        begin
          ...
          else if (m_btRace = 70) {or (m_btRace = 81)} then
          begin
      ...
      else if m_nCurrentAction = SM_LIGHTING_1 then
      begin
        if (m_nCurrentFrame - m_nStartFrame = 4) then
        begin
          if (m_btRace = 114) then
          begin
            g_PlayScene.NewMagic(Self,
              MAGIC_FOX_FIRE1,
              MAGIC_FOX_FIRE1,
  ...
  begin
    if m_btRace in [118, 119] then
    begin
      if GetTickCount - m_dwDefFrameTime > 150 then
      begin
        m_dwDefFrameTime := GetTickCount;
        Inc(m_nCurrentDefFrame);
        if m_nCurrentDefFrame >= m_nDefFrameCount then
...
begin
  ...
  begin
    case m_nCurrentAction of
      ...
      SM_NOWDEATH: if (m_btRace = 62) then
        begin
...
begin
  inherited;
  if m_boNowDeath and (AttackEffectSurface <> nil) then
  begin
...
begin
  ...
  if m_boUseEffect or m_boNowDeath then
  begin
    if m_boMsgMuch then
      m_dwEffectFrameTimetime := Round(m_dwEffectframetime * 2 / 3)
    else
...
begin
  ...
  case m_nCurrentAction of
    ...
    SM_LIGHTING, SM_LIGHTING_1..SM_LIGHTING_3: if m_btRace = 120 then
      begin
        m_nStartFrame := 0;
        m_nEndFrame := 19;
        m_dwFrameTime := 150;
    ...
    SM_STRUCK: if m_btRace <> 120 then
      begin
        m_nStartFrame := 0;
        m_nEndFrame := 9;
        m_dwFrameTime := 300;
  ...
  if m_btRace in [118, 119, 120] then
    m_boUseEffect := True;
end;
...
begin
  ...
  begin
    ...
    else if LightningTimer.Tag = 2 then
    begin
...
begin
  if (m_btRace <> 120) and m_boDeath then
    Exit;
  inherited;
  if m_boUseEffect and (m_DrawEffect <> nil) then
    DrawBlend(dsurface, dx + ax + m_nShiftX, dy + ay + m_nShiftY, m_DrawEffect, 1);
...
begin
  ...
  begin
    if m_btRace = 120 then
    begin
      if ((2900 + m_nCurrentFrame + 20) > 2900 + 419) and ((2900 + m_nCurrentFrame + 20) < 2900 + 438) then
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if (m_nCurrentAction = SM_LIGHTING_1) and (m_nCurrentFrame - m_nStartFrame = 1) then
        begin
          g_PlayScene.NewMagic(Self,
            MAGIC_SOULBALL_ATT2,
            MAGIC_SOULBALL_ATT2,
        ...
        else if (m_nCurrentAction = SM_LIGHTING_2) and (m_nCurrentFrame - m_nStartFrame = 1) then
        begin
          if not LightningTimer.Enabled then
          begin
      ...
      else if (m_nCurrentAction = 81) or (m_nCurrentAction = 82) or (m_nCurrentAction = 83) then
      begin
        if (m_nCurrentFrame - m_nStartFrame) = 4 then
        begin
...
var
  bofly: Boolean;
begin
  if (m_nCurrentAction = SM_LIGHTING) and (boCasted = True) then
  begin
    boCasted := False;
...
begin
  ...
  begin
    ...
    if m_nCurrentDefFrame < 0 then
      cf := 0
    else if m_nCurrentDefFrame >= pm.ActDeath.frame then
      cf := 0
    else
      cf := m_nCurrentDefFrame;
...
begin
  inherited;
  if (m_btRace = 109) and m_boUseEffect and (m_nCurrentAction = SM_LIGHTING) then
  begin
...
begin
  inherited;
  if (m_btRace = 110) and m_boUseEffect and (m_nCurrentAction = SM_LIGHTING) then
  begin
  ...
  case m_nCurrentAction of
    ...
    SM_LIGHTING:
      begin
        ...
        if (m_nMagicNum = 2) and (m_btRace in [38, 39, 46]) then
        begin
          m_nStartFrame := pm.ActDeath.start + m_btDir * (pm.ActDeath.frame + pm.ActDeath.skip);
          m_nEndFrame := m_nStartFrame + pm.ActDeath.frame - 1;
          m_dwFrameTime := pm.ActDeath.ftime;
  ...
  case m_btRace of
    ...
    39:
      begin
        ...
        else if m_wAppearance = 268 then
        begin
          if m_boUseEffect then
          begin
    ...
    46: if m_boUseEffect and (m_nMagicNum = 2) then
      begin
    ...
    51:
      begin
        ...
        if m_boUseEffect then
          AttackEffectSurface := g_WMon27Img.GetCachedImage(nImgBase + (m_btDir * 10) + m_neffectframe - m_nEffectStart, ax, ay);
        if BoUseDieEffect then
          DieEffectSurface := g_WMon27Img.GetCachedImage(1770 + (m_btDir * 10) + m_nCurrentFrame - m_nStartFrame, bx, by);
      end;
  end;
end;
...
var
  dwEffectFrameTimetime: LongWord;
begin
  inherited;
  if m_boUseEffect then
  begin
    ...
    if GetTickCount - m_dwEffectstarttime > dwEffectFrameTimetime then
    begin
      m_dwEffectstarttime := GetTickCount;
      if m_neffectframe < m_nEffectEnd then
      begin
...
begin
  ...
  begin
    ...
    if AttackEffectSurface2 <> nil then
    begin
      DrawBlend(dsurface,
        dx + bx + m_nShiftX,
        dy + by + m_nShiftY,
...
begin
  inherited;
  if (m_btRace = 110) and m_boUseEffect and (m_nCurrentAction = SM_FLYAXE) then
  begin
    AttackEffectSurface := g_WMon24Img.GetCachedImage(1100 + (m_btDir * 10) + m_neffectframe - m_nEffectStart, ax, ay);
  end;
end;
...
begin
  ...
  case m_nCurrentAction of
    ...
    SM_DIGUP:
      begin
        if (m_wAppearance = 330) or (m_wAppearance = 336) then
        begin
          m_nStartFrame := 4;
...
begin
  ...
  case m_nCurrentAction of
        ...
        case m_wAppearance of
          354, 356, 359, 813, 815:
            begin
              ...
              m_dwStartTime := GetTickCount;
              m_dwWarModeTime := GetTickCount;
              if (m_wAppearance = 354) or (m_wAppearance = 815) then
              begin
    ...
    SM_SPELL:
      begin
        if m_CurMagic.MagicSerial = 23 then
        begin
          m_nStartFrame := m_Action.ActCritical.start + m_btDir * (m_Action.ActCritical.frame + m_Action.ActCritical.skip);
          m_nEndFrame := m_nStartFrame + m_Action.ActCritical.frame - 1;
          m_dwFrameTime := m_Action.ActCritical.ftime;
...
begin
  if m_nCurrentAction = SM_LIGHTING then
  begin
    if (frame = 5) and m_boSmiteWideHit2 then
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (m_nCurEffFrame = m_nSpellFrame - 2) then
          begin
