...
begin
  ...
  try
    ...
    nPitch := ddsd.lPitch;                        //lPitch以Byte为单位，GraphPitch以WORD为单位。所以GraphPitch = lPitch / 2;
    ...
    begin
      ...
      begin
        WordColor := lpBuffer^;
        TempMap^ := Byte(GetSurfaceBlue(WordColor));              //  fputc(GetBlue(word), fp);         // 蓝
        Inc(TempMap);
        TempMap^ := Byte(GetSurfaceGreen(WordColor));             //  fputc(GetGreen(word), fp);        // 绿
        Inc(TempMap);
        TempMap^ := Byte(GetSurfaceRed(WordColor));               //  fputc(GetRed(word), fp);          // 红
        Inc(TempMap);
        Inc(lpBuffer);
      end;
      Dec(lpBuffer,nPitch);
...
begin
  ...
  case smode.StdMode of
    ...
    12, 13:   if not g_boUI0508 then  Result := U_FASHION;
    17: if not g_boUI0508 then Result := U_DRUM;
    18: if not g_boUI0508 then Result := U_HORSE;
...
begin
  ...
  if MsgNum = 1 then
    DScreen.AddSysMsg(iname + ' 在英雄包裹内被发现');
