...
begin
  ...
  if (m_nEventType >= CUSTOM_SAFEZONE_EFFSTART_INX) and (m_nEventType <= CUSTOM_SAFEZONE_EFFEND_INX) then //如果事件类型是自定义光圈特效则执行自定义贞间播速 2019-12-06
  begin
      ptPackage := GetSafeZoneEffectCustomConfig(m_nEventType);
      if ptPackage <> nil then
      begin
        if ptPackage^.EventType = m_nEventType then
        begin
          if GetTickCount - m_dwFrameTime > ptPackage^.nEffectsSpeed then
          begin
            m_dwFrameTime := GetTickCount;
            Inc(m_dwCurframe);
          end;
  ...
  begin
    if GetTickCount - m_dwFrameTime > 20 then
    begin
      m_dwFrameTime := GetTickCount;
      Inc(m_dwCurframe);
    end;
...
{$IFEND}
    ...
    CUSTOM_SAFEZONE_EFFSTART_INX .. CUSTOM_SAFEZONE_EFFEND_INX: //CUSTOM_SAFEZONE_EFFSTART_INX..CUSTOM_SAFEZONE_EFFEND_INX 都是自定义安全区效果展示   2019-12-2
      begin
          ptPackage := GetSafeZoneEffectCustomConfig(m_nEventType);
          if ptPackage <> nil then
          begin
            if ptPackage^.EventType = m_nEventType then
            begin
              m_Dsurface := ptPackage^.DrawCustom.GetCachedImage(ptPackage^.nEffectsStartOffset  + (m_dwCurframe mod ptPackage^.nEffectsCountOffset) ,m_nPx,m_nPy);
              m_boBlend :=  ptPackage^.bEffectsBlendDraw;
              m_nLight := 1;
            end;
          end;
      end;
...
begin
  for i := 0 to EventList.count - 1 do
    if (EventList[i] = evn) or (TClEvent(EventList[i]).m_nServerId = evn.m_nServerId) then
    begin
      evn.Free;
      Result := nil;
      Exit;
...
begin
  for i := 0 to EventList.count - 1 do
    if TClEvent(EventList[i]).m_nServerId = svid then
    begin
      TClEvent(EventList[i]).Free;
      EventList.Delete(i);
      Break;
...
begin
  ...
  for i := 0 to EventList.count - 1 do
    if (TClEvent(EventList[i]).m_nX = ax) and (TClEvent(EventList[i]).m_nY = ay) and
      (TClEvent(EventList[i]).m_nEventType = etype) then
    begin
