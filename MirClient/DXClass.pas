...
type
  ...
  private
    ...
    procedure WMSYSCommandEx(var Msg: TWMSYSCommand); message WM_SIZE;
  protected
...
begin
  ...
  //ExStyle := ExStyle or WS_EX_TOOLWINDOW;                   //  最小化窗口不显示在任务栏
...
begin
  ...
  if SetAppExStyleCount = 0 then begin
    ExStyle := GetWindowLong(Application.Handle, GWL_EXSTYLE);
    //ExStyle := ExStyle and (not WS_EX_TOOLWINDOW);         //  最小化窗口不显示在任务栏
    SetWindowLong(Application.Handle, GWL_EXSTYLE, ExStyle);
  end;
  inherited Destroy;
end;
...
procedure TDXForm.WMSYSCommandEx(var Msg: TWMSYSCommand);
begin
  if (Msg.CmdType = SIZE_MINIMIZED) or (Msg.CmdType = SIZE_MAXHIDE) then begin
    DefaultHandler(Msg);
    WindowState := wsMinimized;
    Application.Minimize;
  end else
