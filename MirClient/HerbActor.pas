...
uses
  ...
  Grobal2, DXDraws, cliUtil, magiceff, HumanActor, Actor, WIL;
...
begin
  if (Map.m_nCurUnitX <> oldunitx) or (Map.m_nCurUnitY <> oldunity) then
  begin
    if m_boDeath then
      ApplyDoorState(dsBroken)
...
begin
  ...
  begin //(CurrentAction = SM_NOWDEATH) or (CurrentAction = SM_DEATH) then begin
  ...
  if (m_wAppearance >= 901) and (m_wAppearance <= 903) then
  begin
...
begin
  ...
  if (BrokenSurface <> nil) and (not blend) then
  begin
...
begin
  ...
  begin
    if (m_nCurrentFrame - m_nStartFrame) >= 5 then
    begin
  ...
  if m_boUseEffect then
  begin
    if (GetTickCount - m_dwEffectStartTime) > m_dwEffectFrameTime then
    begin
      m_dwEffectStartTime := GetTickCount();
