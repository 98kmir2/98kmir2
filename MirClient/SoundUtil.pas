...
uses
  ClMain, MShare, Actor, DrawScrn, PatchUnit;
...
begin
  if FSilent or not frmMain.Active then
    Exit;

  if (FileIndex >= 0) and (FileIndex < FSoundList.count) then
  begin
    if FSoundList[FileIndex] <> '' then
    begin
...
begin
  ...
  try
    ...
    begin
      pGsnd := pTGameSound(FList[i]);
      if (pGsnd.Snd <> nil) and (CompareText(pGsnd.Name, FSoundList[FileIndex]) = 0) then
      begin
...
begin
  ...
  try
    ...
    begin
      pGsnd := pTGameSound(FList[i]);
      if (pGsnd.Snd <> nil) and (CompareText(pGsnd.Name, FileName) = 0) then
      begin
        pGsnd.Tick := GetTickCount;
        pGsnd.Pan := GainPanVolume(X, Y, nValue);
        pGsnd.Val := nValue;
...
begin
  ...
  begin
    ...
    try
      ii := 0;
      for i := FNewList.count - 1 downto 0 do
      begin
        if Terminated then Break;
        pGsnd := FNewList[i];
        pGsnd.Snd := nil;
        ...
        begin
          //123456
          if FSendRequestList.IndexOf(pGsnd.Name) < 0 then
          begin
    ...
    try
      ...
      if (FList.count > 0) and (GetTickCount - FCacheTick > 100) then
      begin
        FCacheTick := GetTickCount;
        n := 16;
        while n > 0 do
        begin
          ...
          if (pGsnd.Cnl <> nil) and pGsnd.Cnl.IsPlaying then
          begin
            pGsnd.Tick := GetTickCount;
          ...
          begin
            if GetTickCount - pGsnd.Tick > 8 * 1000 then
            begin
              Dispose(pGsnd);
              pGsnd := nil;
              FList.Delete(0);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (sMapName <> '') and (sFileName <> '') then
      begin
        New(pFileName);
        pFileName^ := sFileName;
...
begin
  ...
  if (not ReadFile(F, data^, nSize, BytesRead, nil)) then
  begin
    CloseHandle(F);
    FreeMem(data);
    Exit;
...
begin
  case Std.StdMode of
    ...
    31: if Std.AniCount in [1..3] then
