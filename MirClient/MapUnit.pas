...
uses
  Windows, Classes, SysUtils, Graphics, Grobal2, HUtil32, DXDraws, cliUtil, MShare;
...
var
  X, Y, n, nMapSize: Integer;
  TempMapInfoArr: pTMapInfoArr_Old;
  TempMapInfoArr2: pTMapInfoArr_2;
  canMove: Boolean;
begin
  if m_nCurrentMap <> 0 then
  begin
    if m_MapBuf = nil then
    begin
      ...
      case Byte(m_MapHeader.Reserved[0]) of
        ...
        2:
          begin
            ...
            for X := 0 to m_MapHeader.wWidth * m_MapHeader.wHeight - 1 do
            begin
...
begin
  Result := False;
  if (g_MySelf <> nil) and (m_nCurrentMap <> 0) and (m_MapBuf <> nil) then
  begin
    ...
    begin
      ...
      if (a.m_nCurrX >= g_MySelf.m_nCurrX - 32) and (a.m_nCurrX <= g_MySelf.m_nCurrX + 32) and
        (a.m_nCurrY >= g_MySelf.m_nCurrY - 32) and (a.m_nCurrY <= g_MySelf.m_nCurrY + 32) then
      begin
...
begin
  ...
  begin
    ...
    if nTy < 0 then
      nTy := 0;
    if nBy >= m_MapHeader.wHeight then
    ...
    case Byte(m_MapHeader.Reserved[0]) of
      6:
        begin
          ...
          begin
            if (i >= 0) and (i < m_MapHeader.wWidth) then
            begin
...
begin
  ...
  begin
    ...
    if m_nCurrentMap <> 0 then
    begin
      if FileRead(m_nCurrentMap, m_MapHeader, SizeOf(TMapHeader)) <> SizeOf(TMapHeader) then
      begin
        FileClose(m_nCurrentMap);
        m_nCurrentMap := 0;
      end;
  ...
  begin
    if FSendRequestList.IndexOf(sFileName) < 0 then
    begin
