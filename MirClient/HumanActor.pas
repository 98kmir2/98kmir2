...
begin
  ...
  begin
    if (g_MagicLockActor = nil) or g_MagicLockActor.m_boDeath then
    begin
      g_SeriesSkillFire := False;
      g_SeriesSkillStep := 0;
    end;
    if m_boUseMagic and (Self = g_MySelf) and (g_MagicLockActor <> nil) and (not g_MagicLockActor.m_boDeath) and (g_nCurrentMagic <= 3) then
    begin
      if (m_nCurrentFrame - m_nStartFrame) >= (m_nSpellFrame - 1) then
      begin
        if g_MagicArr[0][g_SeriesSkillArr[g_nCurrentMagic]] <> nil then
        begin
          frmMain.UseMagic(g_nMouseX, g_nMouseY, g_MagicArr[0][g_SeriesSkillArr[g_nCurrentMagic]], False, True);
        end;
        Inc(g_nCurrentMagic);
        if g_nCurrentMagic > _MIN(3, g_SeriesSkillStep) then
        begin
          g_SeriesSkillFire := False;
          g_SeriesSkillStep := 0;
        end;
...
begin
  ...
  if m_boUseCboLib and m_boHitEffect and (Self = g_MySelf) and (g_nCurrentMagic2 < 4) then
  begin
    if (m_nCurrentFrame - m_nStartFrame) = 2 then
    begin
      ...
      if g_nCurrentMagic2 > _MIN(4, g_SeriesSkillStep) then
      begin
        g_SeriesSkillFire := False;
        //  g_SeriesSkillStep := 0;
      end;
...
begin
  ...
  else if m_btHairEx > 0 then
  begin
     // 限制 m_btHairEx     2019-12-12
    {haircount := g_WHair2ImgImages.ImageCount div HUMANFRAME div 2;
  ...
  case m_nCurrentAction of
    ...
    SM_HIT, SM_POWERHIT, SM_LONGHIT, SM_WIDEHIT, SM_FIREHIT,
      SM_HERO_LONGHIT, SM_HERO_LONGHIT2, SM_SQUHIT, SM_CRSHIT, SM_TWNHIT:
      begin
        m_nStartFrame := HA.ActHit.start + m_btDir * (HA.ActHit.frame + HA.ActHit.skip);
        m_nEndFrame := m_nStartFrame + HA.ActHit.frame - 1;
        m_dwFrameTime := HA.ActHit.ftime;
        ...
        m_boWarMode := True;
        m_dwWarModeTime := GetTickCount;
        if (m_nCurrentAction = SM_POWERHIT) then
        begin
          m_boHitEffect := True;
          m_nMagLight := 2;
        ...
        if (m_nCurrentAction = SM_LONGHIT) then
        begin
          m_boHitEffect := True;
          m_nMagLight := 2;
        ...
        if (m_nCurrentAction = SM_WIDEHIT) then
        begin
          m_boHitEffect := True;
          m_nMagLight := 2;
        ...
        if (m_nCurrentAction = SM_FIREHIT) then
        begin
          m_boHitEffect := True;
          m_nMagLight := 2;
        ...
        if (m_nCurrentAction = SM_HERO_LONGHIT) then
        begin
          m_boHitEffect := True;
          m_nMagLight := 2;
    ...
    SM_WWJATTACK, SM_WSJATTACK, SM_WTJATTACK:
      begin
        ...
        if (m_nCurrentAction = SM_WWJATTACK) then
        begin
        ...
        if (m_nCurrentAction = SM_WSJATTACK) then
        begin //雷霆一击
        ...
        if (m_nCurrentAction = SM_WTJATTACK) then
        begin //劈星斩
    ...
    SM_BIGHIT, SM_PURSUEHIT:
      begin
        ...
        m_boWarMode := True;
        m_dwWarModeTime := GetTickCount;
        if (m_nCurrentAction = SM_PURSUEHIT) then
        begin
          case m_CurMagic.magfirelv div 4 of
    ...
    SM_SPELL:
      begin
        if m_CurMagic.EffectNumber in [104..114] then
        begin
        ...
        begin
          ...
          case m_CurMagic.EffectNumber of
            ...
            15: if m_CurMagic.spelllv > 3 then
              begin
                m_nMagLight := 2;
                m_nSpellFrame := 10;
              end;
...
begin
  ...
  if m_boUseCboLib then
  begin
    if (m_btEffect = 50) then
    begin
      if (m_nCurrentFrame <= 536) then
      begin
        if (GetTickCount - m_dwFrameTick) > 100 then
        begin
          if m_nFrame < 19 then
            Inc(m_nFrame)
          else
    ...
    begin
      ...
      begin
        ...
        if not g_gcGeneral[6] then
        begin
          if m_nCboHumWinOffSet >= 68000 then
          begin
            case m_btEffect of
          ...
          else if m_nCboHumWinOffSet >= 56000 then
      ...
      else if not g_gcGeneral[6] then
      begin
        if m_nCboHumWinOffSet >= 68000 then
        begin
          case m_btEffect of
  ...
  begin
    ...
    begin
      if m_nCurrentFrame < 64 then
      begin
        if (GetTickCount - m_dwFrameTick) > HUMWINEFFECTTICK then
        begin //Blue
          if m_nFrame < 7 then
            Inc(m_nFrame)
          else
...
begin
  ...
  begin
    ...
    begin
      ...
      if HeroCharEffect <> nil then
        g_PlayScene.m_EffectList.Add(HeroCharEffect);
    end;
  end
  else if m_nCurrentAction = SM_SMITELONGHIT then
  begin
    if (frame = 4) and (m_boSmiteLongHit = 2) then
    begin
      m_boSmiteLongHit := 0;
      ...
      if neff <> nil then
        g_PlayScene.m_EffectList.Add(neff);
      g_ShakeScreen.SetScrShake_X(2);
      g_ShakeScreen.SetScrShake_Y(2);
    end;
  end
  else if m_nCurrentAction = SM_SMITELONGHIT3 then
  begin
    if (frame = 4) then
    begin
  ...
  else if m_nCurrentAction = SM_SMITEWIDEHIT2 then
  begin
    if (frame = 8) and (m_boSmiteWideHitS2 > 0) then
    begin
      Dec(m_boSmiteWideHitS2);
      g_SndMgr.PlaySound('Wav\squarehit.wav');
    end;
    if (frame = 10) and m_boSmiteWideHit2 then
    begin
  ...
  else if m_nCurrentAction = SM_SMITELONGHIT2 then
  begin
    if (frame = 8) and (m_boSmiteLongHitS2 > 0) then
    begin
      Dec(m_boSmiteLongHitS2);
      g_SndMgr.PlaySound('Wav\squarehit.wav');
    end;
    if (frame = 11) and m_boSmiteLongHit2 then
    begin
  ...
  else if m_nCurrentAction = SM_SMITEHIT then
  begin
    if (frame = 14) and (m_boSmiteHit) then
    begin
      m_boSmiteHit := False;
  ...
  else if m_nCurrentAction = SM_HERO_LONGHIT2 then
  begin
    if (frame = 4) and (m_boHeroLongHit2) then
    begin
  ...
  else if m_nCurrentAction = SM_HERO_LONGHIT then
  begin
    if (frame = 4) and (m_boHeroLongHit) then
    begin
  ...
  else if m_nCurrentAction = SM_HEAVYHIT then
  begin
  ...
  else if m_nCurrentAction = SM_SITDOWN then
  begin
    if (frame = 5) and (m_boDigFragment) then
    begin
      m_boDigFragment := False;
  ...
  else if m_nCurrentAction = SM_THROW then
  begin
    if (frame = 3) and (m_boThrow) then
    begin
      m_boThrow := False;
...
begin
  ...
  if (m_nCurrentAction in [5..7, 9, 11, 13, 39]) then
    Exit;
  ...
  begin
    ...
    else if (Self <> g_MySelf) and (m_boUseMagic) then
    begin
      if m_boUseCboLib then
      begin
        if m_btIsHero = 1 then
    ...
    if g_boSpeedRate then
      dwFrameTimetime := _Max(0, dwFrameTimetime - _MIN(10, g_MoveSpeedRate));
    ...
    begin
      ...
      begin
        //bss := m_CurMagic.EffectNumber in [105..107, 109..111];
        if bss and (m_CurMagic.ServerMagicCode > 0) then
        begin
        ...
        if fAddNewMagic then
        begin
          if (m_CurMagic.ServerMagicCode > 0) and (not bss or m_boNewMagic) then
          begin
            with m_CurMagic do
            begin
              g_PlayScene.NewMagic(Self,
...
begin
  ...
  if m_boUseCboLib and (m_nCurrentAction = SM_SMITELONGHIT) and (m_boSmiteLongHit = 1) then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        case m_btHairEx of
          3..9: m_HairSurface := g_WHair2ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          10..19: m_HairSurface := g_WHair3ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          20..29: m_HairSurface := g_WHair4ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          30..39: m_HairSurface := g_WHair5ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          100..109: m_HairSurface := g_WHair10ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          110..119: m_HairSurface := g_WHair11ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          120..129: m_HairSurface := g_WHair12ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          130..139: m_HairSurface := g_WHair13ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          140..149: m_HairSurface := g_WHair14ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
          150..159: m_HairSurface := g_WHair15ImgImages.GetCachedImage(m_nHairOffsetEx + m_nCurrentFrame, m_nHpx, m_nHpy);
        else
          m_HairSurface := nil;
        end;
        // m_HairSurface := g_Resource98k.Images[80];
        // m_HairSurface := g_WHair11ImgImages.GetCachedImage(4800, m_nHpx, m_nHpy)
      end
      else if m_nHairOffset >= 0 then
  ...
  begin
    if (m_btEffect = 50) then
    begin
      if (GetDefaultFrame(True) <= 536) then
      begin
        if (GetTickCount - m_dwFrameTick) > 100 then
        begin
          if m_nFrame < 19 then
    ...
    begin
      if GetDefaultFrame(True) < 64 then
      begin
        if (GetTickCount - m_dwFrameTick) > HUMWINEFFECTTICK {200} then
        begin //Blue
          if m_nFrame < 7 then
...
begin
  ...
  if (Self <> g_MySelf) and (Self <> g_MySelf.m_HeroObject) and (m_sUserName = '') and (GetTickCount - m_dwSendQueryUserNameTime > 120 * 1000) then
  begin
  ...
  if m_boUseCboLib and ((m_nState and $04000000 <> 0) or (m_nState and $02000000 <> 0)) then
    m_boUseCboLib := False;
  ...
  //Stall
  if DrawOnSale and m_StallMgr.OnSale then
  begin
    if not m_btDir in [1, 3, 5, 7] then
      m_btDir := 5;
    dd := nil;
    case m_btDir of
      1:
  ...
  if (m_btEffect <> 0) and (m_HumWinSurface <> nil) then
  begin
    ...
    begin
      if blend then
      begin
        if ((m_btDir in [3..5]) or bWarMode) and not boFlag then
        begin
          DrawBlend(dsurface,
            ShiftX + m_nSpx,
            ShiftY + m_nSpy,
        ...
        else if ((m_btDir in [3..5]) or bWarMode) and boFlag then
        begin
          DrawBlend(dsurface,
            ShiftX + m_nSpx,
            ShiftY + m_nSpy,
    ...
    begin
      if ((g_FocusCret <> nil) or (g_MagicTarget <> nil)) and blend and ((m_btDir in [3..5]) or bWarMode) and not boFlag then
      begin
        DrawBlend(dsurface,
          ShiftX + m_nSpx,
          ShiftY + m_nSpy,
  ...
  if bWeapon and (m_nWpord = 0) and (not blend) then
  begin
  ...
  if (m_nWpord = 1) and bWeapon then
  begin
    DrawEffSurface(dsurface, m_WeaponSurface, ShiftX + m_nWpx, ShiftY + m_nWpy, blend, ceNone);
    //DrawWeaponGlimmer(dsurface, dx + m_nShiftX, dy + m_nShiftY);
    if (m_WeaponEffect <> nil) then
  ...
  else if (m_btEffect <> 0) and (m_HumWinSurface <> nil) then
  begin
    if g_MySelf = Self then
    begin
      if blend then
      begin
        if not bWarMode and (m_btDir in [0..2, 6, 7]) and not boFlag then
        begin
          DrawBlend(dsurface,
            ShiftX + m_nSpx,
            ShiftY + m_nSpy,
        ...
        else if not bWarMode and (m_btDir in [0..2, 6, 7]) and boFlag then
        begin
          DrawBlend(dsurface,
            ShiftX + m_nSpx,
            ShiftY + m_nSpy,
    ...
    begin
      if ((g_FocusCret <> nil) or (g_MagicTarget <> nil)) and not bWarMode and (m_btDir in [0..2, 6, 7]) and not boFlag then
      begin
        DrawBlend(dsurface,
          ShiftX + m_nSpx,
          ShiftY + m_nSpy,
      ...
      begin
        if not bWarMode and (m_btDir in [0..2, 6, 7]) and boFlag then
        begin
          DrawBlend(dsurface,
            ShiftX + m_nSpx,
            ShiftY + m_nSpy,
  ...
  if not m_boDeath and not m_boUseCboLib then
  begin
    if m_nState and $00100000 <> 0 then
    begin //显示魔法盾时效果
    ...
    if m_nState and $00080000 <> 0 then
    begin //显示4级魔法盾时效果
    ...
    //其他附身效果
    if m_nState and $00040000 <> 0 then
    begin
    ...
    if m_nState and $00020000 <> 0 then
    begin //龙化
  ...
  begin //sm_spell
    ...
    begin
      ...
      begin
        if (m_CurMagic.EffectNumber in [29]) then
        begin
          if m_CurMagic.spelllv > MAXMAGICLV then
        ...
        begin
          if (m_CurMagic.EffectNumber in [4, 8..12, 15, 20, 21, 28, 31, 34, 48, 51]) then
          begin
  ...
  begin
    ...
    else if (m_nHitEffectNumber > 0) then
    begin
      //
    ...
    begin
      ...
      begin
        if ((g_FocusCret <> nil) or (g_MagicTarget <> nil)) and
          not bWarMode and (m_btDir in [0..2, 6, 7]) and not boFlag then
        begin
          DrawBlend(dsurface,
            dx + m_nSpx + m_nShiftX,
            dy + m_nSpy + m_nShiftY,
